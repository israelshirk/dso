...
void
KeyFrameDisplay::setFromF(FrameShell* frame, CalibHessian* HCalib)
{
	id = frame->id;
	fx = HCalib->fxl();
	fy = HCalib->fyl();
...
void
KeyFrameDisplay::setFromKF(FrameHessian* fh, CalibHessian* HCalib)
{
	...
	for (ImmaturePoint* p : fh->immaturePoints) {
		...
		pc[numSparsePoints].idpeth =
		  (p->idepth_max + p->idepth_min) * 0.5f;
...
bool
KeyFrameDisplay::refreshPC(bool canRefresh,
			   float scaledTH,
			   float absTH,
			   int mode,
			   float minBS,
			   int sparsity)
{
	...
	for (int i = 0; i < numSparsePoints; i++) {
		...
		depth4 *= depth4;
		float var =
		  (1.0f / (originalInputSparse[i].idepth_hessian + 0.01));
		...
		for (int pnt = 0; pnt < patternNum; pnt++) {
			...
			if (my_displayMode == 0) {
				if (originalInputSparse[i].status == 0) {
					tmpColorBuffer[vertexBufferNumPoints]
						      [0] = 0;
					tmpColorBuffer[vertexBufferNumPoints]
						      [1] = 255;
					tmpColorBuffer[vertexBufferNumPoints]
			...
			assert(vertexBufferNumPoints <=
			       numSparsePoints * patternNum);
		}
	}
	...
	vertexBuffer.Upload(
	  tmpVertexBuffer, sizeof(float) * 3 * numGLBufferGoodPoints, 0);
	colorBuffer.Upload(
	  tmpColorBuffer, sizeof(unsigned char) * 3 * numGLBufferGoodPoints, 0);
	bufferValid = true;
	delete[] tmpColorBuffer;
	delete[] tmpVertexBuffer;
...
void
KeyFrameDisplay::drawPC(float pointSize)
{
	...
	colorBuffer.Bind();
	glColorPointer(
	  colorBuffer.count_per_element, colorBuffer.datatype, 0, 0);
	glEnableClientState(GL_COLOR_ARRAY);

	vertexBuffer.Bind();
	glVertexPointer(
	  vertexBuffer.count_per_element, vertexBuffer.datatype, 0, 0);
