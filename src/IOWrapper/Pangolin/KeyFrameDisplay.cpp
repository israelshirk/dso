...
void KeyFrameDisplay::setFromKF(FrameHessian *fh, CalibHessian *HCalib) {
        ...
        if (numSparseBufferSize < npoints) {
                ...
                originalInputSparse = new InputPointSparse<
                    MAX_RES_PER_POINT>[numSparseBufferSize];
        }
...
bool KeyFrameDisplay::refreshPC(bool canRefresh, float scaledTH, float absTH,
        ...
        for (int i = 0; i < numSparsePoints; i++) {
                ...
                for (int pnt = 0; pnt < patternNum; pnt++) {
                        ...
                        tmpVertexBuffer[vertexBufferNumPoints][0] =
                            ((originalInputSparse[i].u + dx) * fxi + cxi) *
                            depth;
                        tmpVertexBuffer[vertexBufferNumPoints][1] =
                            ((originalInputSparse[i].v + dy) * fyi + cyi) *
                            depth;
        ...
        if (numGLBufferGoodPoints > numGLBufferPoints) {
                ...
                vertexBuffer.Reinitialise(pangolin::GlArrayBuffer,
                                          numGLBufferPoints, GL_FLOAT, 3,
                                          GL_DYNAMIC_DRAW);
                colorBuffer.Reinitialise(pangolin::GlArrayBuffer,
                                         numGLBufferPoints, GL_UNSIGNED_BYTE, 3,
                                         GL_DYNAMIC_DRAW);
        }
