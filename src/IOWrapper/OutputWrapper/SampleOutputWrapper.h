   ...
   public:
    ...
    virtual void publishGraph(
        ...
        for (const std::pair<uint64_t, Eigen::Vector2i>& p : connectivity) {
            ...
            printf(
                "OUT: Example Edge %d -> %d has %d active and %d marg "
                "residuals\n",
    ...
    virtual void publishKeyframes(std::vector<FrameHessian*>& frames,
        ...
        for (FrameHessian* f : frames) {
            printf(
                "OUT: KF %d (%s) (id %d, tme %f): %d active, %d marginalized, "
                "%d immature points. CameraToWorld:\n",
            ...
            for (PointHessian* p : f->pointHessians) {
                printf(
                    "OUT: Example Point x=%.1f, y=%.1f, idepth=%f, idepth "
    ...
    virtual void pushDepthImageFloat(MinimalImageF* image, FrameHessian* KF) {
        printf(
            "OUT: Predicted depth for KF %d (id %d, time %f, internal frame-ID "
            "%d). CameraToWorld:\n",
