...
class SampleOutputWrapper : public Output3DWrapper {
	...
	virtual void publishGraph(
		...
		for (const std::pair<uint64_t, Eigen::Vector2i>& p :
		     connectivity) {
			int idHost = p.first >> 32;
			int idTarget = p.first & ((uint64_t)0xFFFFFFFF);
			printf(
			    "OUT: Example Edge %d -> %d has %d active and %d "
			    "marg residuals\n",
	...
	virtual void publishKeyframes(std::vector<FrameHessian*>& frames,
		...
		for (FrameHessian* f : frames) {
			printf(
			    "OUT: KF %d (%s) (id %d, tme %f): %d active, %d "
			    "marginalized, %d immature points. "
			    "CameraToWorld:\n",
			...
			for (PointHessian* p : f->pointHessians) {
				printf(
				    "OUT: Example Point x=%.1f, y=%.1f, "
				    "idepth=%f, idepth std.dev. %f, %d "
				    "inlier-residuals\n",
	...
	virtual void publishCamPose(FrameShell* frame, CalibHessian* HCalib) {
		printf(
		    "OUT: Current Frame %d (time %f, internal ID %d). "
		    "CameraToWorld:\n",
	...
	virtual void pushDepthImageFloat(MinimalImageF* image,
					 FrameHessian* KF) {
		printf(
		    "OUT: Predicted depth for KF %d (id %d, time %f, internal "
		...
		for (int y = 0; y < image->h; y++) {
			for (int x = 0; x < image->w; x++) {
				...
				printf(
				    "OUT: Example Idepth at pixel (%d,%d): "
				    "%f.\n",
				    x, y, image->at(x, y));
				maxWrite--;
				if (maxWrite == 0)
