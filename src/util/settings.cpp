...
bool setting_realTimeMaxKF = false; // if true, takes as many KF's as possible
				    // (will break the system if the camera
				    // stays stationary)
...
float setting_kfGlobalWeight = 1; // general weight on threshold, the larger the
				  // more KF's are taken (e.g., 2 = double the
				  // amount of KF's).
...
float setting_maxLogAffFacInWindow = 0.7; // marg a frame if factor between
					  // intensities to current frame is
...
float setting_margWeightFac = 0.5 * 0.5; // factor on hessian when
					 // marginalizing, to account for
...
int setting_gammaWeightsPixelSelect = 1; // 1 = use original intensity for pixel
					 // selection; 0 = use gamma-corrected
					 // intensity.
...
float setting_maxPixSearch = 0.027; // max length of the ep. line segment
				    // searched during immature point tracking.
...
float setting_trace_extraSlackOnTH = 1.2; // for energy-based outlier check, be
					  // slightly more relaxed by this
					  // factor.
