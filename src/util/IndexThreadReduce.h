...
class IndexThreadReduce
{
	...
	inline IndexThreadReduce()
	{
		...
		stepSize = 1;
		callPerIndex =
		  boost::bind(&IndexThreadReduce::callPerIndexDefault,
			      this,
			      _1,
			      _2,
			      _3,
		...
		for (int i = 0; i < NUM_THREADS; i++) {
			...
			workerThreads[i] = boost::thread(
			  &IndexThreadReduce::workerLoop, this, i);
		}
	}
	inline ~IndexThreadReduce()
	{
	  ...
	  int stepSize = 0)
	{
		...
		if (stepSize == 0)
			stepSize =
			  ((end - first) + NUM_THREADS - 1) / NUM_THREADS;
	...
	void workerLoop(int idx)
	{
		...
		while (running) {
			...
			if (gotSomething) {
				...
				callPerIndex(
				  todo,
				  std::min(todo + stepSize, maxIndex),
				  &s,
				  idx);
				gotOne[idx] = true;
				lock.lock();
			...
			// otherwise wait on signal, releasing lock in the
			// meantime.
			else {
				if (!gotOne[idx]) {
					lock.unlock();
