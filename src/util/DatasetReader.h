...
class ImageFolderReader {
       public:
        ImageFolderReader(std::string path,
                ...
                if (isZipped) {
                        ...
                        int ziperror = 0;
                        ziparchive =
                            zip_open(path.c_str(), ZIP_RDONLY, &ziperror);
                        if (ziperror != 0) {
                        ...
                        for (int k = 0; k < numEntries; k++) {
                                const char* name = zip_get_name(
                                    ziparchive, k, ZIP_FL_ENC_STRICT);
...
#else
                        printf(
                            "ERROR: cannot read .zip archive, as compile "
                            "without ziplib!\n");
                        exit(1);
#endif
                } else
       ...
       private:
        MinimalImageB* getImageRaw_internal(int id, int unused) {
                if (!isZipped) {
                        ...
                        if (readbytes > (long)widthOrg * heightOrg * 6) {
                                ...
                                delete[] databuffer;
                                databuffer =
                                    new char[(long)widthOrg * heightOrg * 30];
                                fle =
                                    zip_fopen(ziparchive, files[id].c_str(), 0);
                                readbytes = zip_fread(
                                    fle, databuffer,
                                ...
                                if (readbytes >
                                    (long)widthOrg * heightOrg * 30) {
                                        printf(
                                            "buffer still to small (read "
                                            "%ld/%ld). abort.\n",
                                            readbytes,
                                            (long)widthOrg * heightOrg * 30 +
                                                10000);
        ...
        inline void loadTimestamps() {
                ...
                while (!tr.eof() && tr.good()) {
                        ...
                        if (3 ==
                            sscanf(buf, "%d %lf %f", &id, &stamp, &exposure)) {
