...
class PhotometricUndistorter
{
	...
	PhotometricUndistorter(std::string file,
...
class Undistort
{
	...
	virtual void distortCoordinates(float* in_x,
					float* in_y,
					float* out_x,
					float* out_y,
					int n) const = 0;
	...
	void loadPhotometricCalibration(std::string file,
					std::string noiseImage,
					std::string vignetteImage);
	...
	void readFromFile(const char* configFileName,
			  int nPars,
			  std::string prefix = "");
};
...
class UndistortFOV : public Undistort
{
	...
	~UndistortFOV();
	void distortCoordinates(float* in_x,
				float* in_y,
				float* out_x,
				float* out_y,
				int n) const;
