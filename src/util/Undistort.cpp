...
PhotometricUndistorter::PhotometricUndistorter(std::string file,
...

	{
		...
		std::vector<float> Gvec =
		    std::vector<float>(std::istream_iterator<float>(l1i),
		...
		if (GDepth < 256) {
			printf(
			    "PhotometricUndistorter: invalid format! got %d "
		...
		for (int i = 0; i < GDepth - 1; i++) {
			if (G[i + 1] <= G[i]) {
				printf(
				    "PhotometricUndistorter: G invalid! it has "
				    "to be strictly increasing, but it "
				    "isnt!\n");
				return;
			}
		}
	...
	if (vm16 != 0) {
		if (vm16->w != w || vm16->h != h) {
			printf(
			    "PhotometricUndistorter: Invalid vignette image "
			    "size! got %d x %d, expected %d x %d\n",
			    vm16->w, vm16->h, w, h);
			if (vm16 != 0)
				delete vm16;
			if (vm8 != 0)
...
template <typename T>
void PhotometricUndistorter::processFrame(T* image_in,
					  float exposure_time,
					  float factor) {
	int wh = w * h;
...
Undistort* Undistort::getUndistorterForFile(std::string configFilename,
	...
	if (!f.good()) {
		f.close();
		printf(
		    " ... not found. Cannot operate without calibration, "
		    "shutting down.\n");
		f.close();
		return 0;
	}
	...
	if (std::sscanf(l1.c_str(), "%f %f %f %f %f %f %f %f", &ic[0], &ic[1],
		...
		printf(
		    "found RadTan (OpenCV) camera model, building "
		    "rectifier.\n");
	...
	else if (std::sscanf(l1.c_str(), "%f %f %f %f %f", &ic[0], &ic[1],
			     &ic[2], &ic[3], &ic[4]) == 5) {
		if (ic[4] == 0) {
			printf(
			    "found PINHOLE camera model, building "
			    "rectifier.\n");
	...
	else if (std::sscanf(l1.c_str(),
			     "KannalaBrandt %f %f %f %f %f %f %f %f", &ic[0],
	...
	else if (std::sscanf(l1.c_str(), "Pinhole %f %f %f %f %f", &ic[0],
			     &ic[1], &ic[2], &ic[3], &ic[4]) == 5) {
...
void Undistort::loadPhotometricCalibration(std::string file,
					   std::string noiseImage,
					   std::string vignetteImage) {
	photometricUndist = new PhotometricUndistorter(
	    file, noiseImage, vignetteImage, getOriginalSize()[0],
	    getOriginalSize()[1]);
}
...
ImageAndExposure* Undistort::undistort(const MinimalImage<T>* image_raw,
	...
	if (image_raw->w != wOrg || image_raw->h != hOrg) {
		printf(
		    "Undistort::undistort: wrong image size (%d %d instead of "
		    "%d %d) \n",
	...
	if (!passthrough) {
		...
		if (benchmark_varNoise > 0) {
			int numnoise = (benchmark_noiseGridsize + 8) *
...
template ImageAndExposure* Undistort::undistort<unsigned char>(
    const MinimalImage<unsigned char>* image_raw,
    float exposure,
    double timestamp,
    float factor) const;
template ImageAndExposure* Undistort::undistort<unsigned short>(
    const MinimalImage<unsigned short>* image_raw,
    float exposure,
    double timestamp,
    float factor) const;
...
void Undistort::applyBlurNoise(float* img) const {
	...
	if (benchmark_varBlurNoise > 0) {
		for (int i = 0; i < numnoise; i++) {
			noiseMapX[i] =
			    benchmark_varBlurNoise * (rand() / (float)RAND_MAX);
			noiseMapY[i] =
			    benchmark_varBlurNoise * (rand() / (float)RAND_MAX);
		}
	}
...
void Undistort::makeOptimalK_crop() {
	...
	// side that has invalid pixels, if several to choose from, shrink the
	// wider dimension.
	...
	while (oobLeft || oobRight || oobTop || oobBottom) {
		...
		if ((oobLeft || oobRight) && (oobTop || oobBottom)) {
			if ((maxX - minX) > (maxY - minY))
				oobBottom = oobTop =
				    false;  // only shrink left/right
			else
				oobLeft = oobRight =
				    false;  // only shrink top/bottom
		}
		...
		printf(
		    "iteration %05d: range: x: %.4f - %.4f; y: %.4f - %.4f!\n",
		    iteration, minX, maxX, minY, maxY);
		if (iteration > 500) {
			printf(
			    "FAILED TO COMPUTE GOOD CAMERA MATRIX - SOMETHING "
			    "IS SERIOUSLY WRONG. ABORTING \n");
			exit(1);
		}
	}
...
void Undistort::readFromFile(const char* configFileName,
			     int nPars,
			     std::string prefix) {
	photometricUndist = 0;
	valid = false;
	...
	{
		char buf[1000];
		snprintf(buf, 1000, "%s%%lf %%lf %%lf %%lf %%lf",
			 prefix.c_str());

		if (std::sscanf(l1.c_str(), buf, &parsOrg[0], &parsOrg[1],
				&parsOrg[2], &parsOrg[3], &parsOrg[4]) == 5 &&
		    std::sscanf(l2.c_str(), "%d %d", &wOrg, &hOrg) == 2) {
			printf("Input resolution: %d %d\n", wOrg, hOrg);
			printf("In: %f %f %f %f %f\n", parsOrg[0], parsOrg[1],
			       parsOrg[2], parsOrg[3], parsOrg[4]);
		} else {
			printf(
			    "Failed to read camera calibration (invalid "
			    "format?)\nCalibration file: %s\n",
			    configFileName);
			infile.close();
			return;
	...
	} else if (nPars == 8)  // KB, equi & radtan model
	{
		...
		if (std::sscanf(l1.c_str(), buf, &parsOrg[0], &parsOrg[1],
			...
			printf("In: %s%f %f %f %f %f %f %f %f\n",
			       prefix.c_str(), parsOrg[0], parsOrg[1],
		...
		printf(
		    "called with invalid number of parameters.... forgot to "
		    "implement me?\n");
		infile.close();
		return;
	}
	...
	if (parsOrg[2] < 1 && parsOrg[3] < 1) {
		printf(
		    ...
		    "is the \"relative\" calibration file format,"
		    "and will rescale this by image width / height to fx=%f, "
		    "fy=%f, cx=%f, cy=%f.\n\n",
		    parsOrg[0], parsOrg[1], parsOrg[2], parsOrg[3],
		    parsOrg[0] * wOrg, parsOrg[1] * hOrg,
		    parsOrg[2] * wOrg - 0.5, parsOrg[3] * hOrg - 0.5);

		// rescale and substract 0.5 offset.
		// the 0.5 is because I'm assuming the calibration is given such
		// that the pixel at (0,0) contains the integral over intensity
		// over [0,0]-[1,1], whereas I assume the pixel (0,0) to contain
		// a sample of the intensity ot [0,0], which is best
		// approximated by the integral over
	...
	if (std::sscanf(l4.c_str(), "%d %d", &w, &h) == 2) {
		if (benchmarkSetting_width != 0) {
			...
			if (outputCalibration[0] == -3)
				outputCalibration[0] =
				    -1;  // crop instead of none, since probably
					 // resolution changed.
		}
		...
		printf(
		    "Out: Failed to Read Output resolution... not "
		    "rectifying.\n");
		valid = false;
	}
	...
	if (outputCalibration[0] == -1)
		...
		if (w != wOrg || h != hOrg) {
			printf(
			    "ERROR: rectification mode none requires input and "
			    "output dimenstions to match!\n\n");
			exit(1);
		}
		K.setIdentity();
		K(0, 0) = parsOrg[0];
		...
		if (outputCalibration[2] > 1 || outputCalibration[3] > 1) {
			printf(
			    "\n\n\nWARNING: given output calibration (%f %f %f "
			    "%f) seems wrong. It needs to be relative to image "
			    "width / height!\n\n\n",
			    outputCalibration[0], outputCalibration[1],
			    outputCalibration[2], outputCalibration[3]);
		}
	...
	if (benchmarkSetting_fxfyfac != 0) {
		...
		passthrough = false;  // cannot pass through when fx / fy have
				      // been overwritten.
	}
...
void UndistortFOV::distortCoordinates(float* in_x,
				      float* in_y,
				      float* out_x,
				      float* out_y,
				      int n) const {
	...
	for (int i = 0; i < n; i++) {
		...
		float fac =
		    (r == 0 || dist == 0) ? 1 : atanf(r * d2t) / (dist * r);
...
void UndistortRadTan::distortCoordinates(float* in_x,
					 float* in_y,
					 float* out_x,
					 float* out_y,
					 int n) const {
...
void UndistortEquidistant::distortCoordinates(float* in_x,
					      float* in_y,
					      float* out_x,
					      float* out_y,
					      int n) const {
	...
	for (int i = 0; i < n; i++) {
		...
		float thetad = theta * (1 + k1 * theta2 + k2 * theta4 +
					k3 * theta6 + k4 * theta8);
...
void UndistortKB::distortCoordinates(float* in_x,
				     float* in_y,
				     float* out_x,
				     float* out_y,
				     int n) const {
...
void UndistortPinhole::distortCoordinates(float* in_x,
					  float* in_y,
					  float* out_x,
					  float* out_y,
					  int n) const {
