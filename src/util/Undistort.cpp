					       ...
					       int h_)
{
...

	{
		...
		if (GDepth < 256) {
			printf("PhotometricUndistorter: invalid format! got %d "
			       "entries in first line, expected at least "
			       "256!\n",
			       (int)Gvec.size());
			return;
		}
		...
		for (int i = 0; i < GDepth - 1; i++) {
			if (G[i + 1] <= G[i]) {
				printf("PhotometricUndistorter: G invalid! it "
				       "has to be strictly increasing, but it "
				       "isnt!\n");
				return;
			}
		}
...
void
PhotometricUndistorter::unMapFloatImage(float* image)
{
	int wh = w * h;
	for (int i = 0; i < wh; i++) {
		float BinvC;
...
template<typename T>
void
PhotometricUndistorter::processFrame(T* image_in,
				     float exposure_time,
				     float factor)
{
	int wh = w * h;
...
template void
PhotometricUndistorter::processFrame<unsigned char>(unsigned char* image_in,
						    float exposure_time,
						    float factor);
template void
PhotometricUndistorter::processFrame<unsigned short>(unsigned short* image_in,
						     float exposure_time,
						     float factor);
				 ...
				 std::string vignetteFilename)
{
	...
	if (!f.good()) {
		f.close();
		printf(" ... not found. Cannot operate without calibration, "
		       "shutting down.\n");
		f.close();
		return 0;
	}
...
void
Undistort::loadPhotometricCalibration(std::string file,
				      std::string noiseImage,
				      std::string vignetteImage)
{
	photometricUndist = new PhotometricUndistorter(file,
						       noiseImage,
						       vignetteImage,
						       getOriginalSize()[0],
						       getOriginalSize()[1]);
...
template<typename T>
ImageAndExposure*
Undistort::undistort(const MinimalImage<T>* image_raw,
		     float exposure,
		     double timestamp,
		     float factor) const
{
	if (image_raw->w != wOrg || image_raw->h != hOrg) {
		printf("Undistort::undistort: wrong image size (%d %d instead "
		       "of %d %d) \n",
		       image_raw->w,
		       image_raw->h,
		       w,
	...
	if (!passthrough) {
		...
		memcpy(result->image,
		       photometricUndist->output->image,
		       sizeof(float) * w * h);
	}
...
Undistort::undistort<unsigned char>(
  ...
  float factor) const;
template ImageAndExposure*
Undistort::undistort<unsigned short>(
...
void
Undistort::applyBlurNoise(float* img) const
{
...
Undistort::makeOptimalK_crop()
{
	...
	while (oobLeft || oobRight || oobTop || oobBottom) {
		...
		printf(
		  ...
		  maxY);
		if (iteration > 500) {
			printf("FAILED TO COMPUTE GOOD CAMERA MATRIX - "
...
void
Undistort::readFromFile(const char* configFileName,
			int nPars,
			std::string prefix)
{
	photometricUndist = 0;
	...
	if (nPars == 5) // fov model
	{
		char buf[1000];
		snprintf(
		  buf, 1000, "%s%%lf %%lf %%lf %%lf %%lf", prefix.c_str());
	...
	if (parsOrg[2] < 1 && parsOrg[3] < 1) {
		printf("\n\nFound fx=%f, fy=%f, cx=%f, cy=%f.\n I'm assuming "
		       "this is the \"relative\" calibration file format,"
		       "and will rescale this by image width / height to "
		       "fx=%f, fy=%f, cx=%f, cy=%f.\n\n",
		       parsOrg[0],
		       parsOrg[1],
		       parsOrg[2],
	...
	if (outputCalibration[0] == -1)
		...
		if (w != wOrg || h != hOrg) {
			printf("ERROR: rectification mode none requires input "
			       "and output dimenstions to match!\n\n");
			exit(1);
		}
		K.setIdentity();
		K(0, 0) = parsOrg[0];
		...
		if (outputCalibration[2] > 1 || outputCalibration[3] > 1) {
			printf("\n\n\nWARNING: given output calibration (%f %f "
			       "%f %f) seems wrong. It needs to be relative to "
			       "image width / height!\n\n\n",
			       outputCalibration[0],
			       outputCalibration[1],
			       outputCalibration[2],
...
void
UndistortFOV::distortCoordinates(float* in_x,
				 float* in_y,
				 float* out_x,
				 float* out_y,
				 int n) const
...
void
UndistortRadTan::distortCoordinates(float* in_x,
				    float* in_y,
				    float* out_x,
				    float* out_y,
				    int n) const
...
void
UndistortEquidistant::distortCoordinates(float* in_x,
					 float* in_y,
					 float* out_x,
					 float* out_y,
					 int n) const
...
void
UndistortKB::distortCoordinates(float* in_x,
				float* in_y,
				float* out_x,
				float* out_y,
				int n) const
...
void
UndistortPinhole::distortCoordinates(float* in_x,
				     float* in_y,
				     float* out_x,
				     float* out_y,
				     int n) const
