...
getInterpolatedElement(const float* const mat,
		       const float x,
		       const float y,
		       const int width)
{
...
getInterpolatedElement43(const Eigen::Vector4f* const mat,
			 const float x,
			 const float y,
			 const int width)
{
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f
getInterpolatedElement33(const Eigen::Vector3f* const mat,
			 const float x,
			 const float y,
			 const int width)
{
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f
getInterpolatedElement33OverAnd(const Eigen::Vector3f* const mat,
				const bool* overMat,
				const float x,
				const float y,
				const int width,
...
EIGEN_ALWAYS_INLINE float
getInterpolatedElement11BiCub(const float* const mat,
			      const float x,
			      const float y,
			      const int width)
{
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f
getInterpolatedElement13BiCub(const float* const mat,
			      const float x,
			      const float y,
			      const int width)
{
	...
	return Eigen::Vector3f(
	  v[0], getInterpolatedElement11Cub(grad, dy), v[1]);
}

EIGEN_ALWAYS_INLINE Eigen::Vector3f
getInterpolatedElement33BiCub(const Eigen::Vector3f* const mat,
			      const float x,
			      const float y,
			      const int width)
{
...
getInterpolatedElement44(const Eigen::Vector4f* const mat,
			 const float x,
			 const float y,
			 const int width)
{
...
EIGEN_ALWAYS_INLINE Eigen::Vector2f
getInterpolatedElement42(const Eigen::Vector4f* const mat,
			 const float x,
			 const float y,
			 const int width)
{
...
makeJet3B(float id)
{
	...
	if (icP == 4)
		return Vec3b(
		  255 * (0.5 - 0.5 * ifP), 255, 255 * (0.5 + 0.5 * ifP));
	if (icP == 5)
...
inline Vec3b
makeRedGreen3B(float val) // 0 = red, 1=green, 0.5=yellow.
{
	if (val < 0)
