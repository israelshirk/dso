...
EFResidual::takeDataF()
{
	...
	for (int i = 0; i < 6; i++)
		JpJdF[i] =
		  J->Jpdxi[0][i] * JI_JI_Jd[0] + J->Jpdxi[1][i] * JI_JI_Jd[1];
...
EFFrame::takeData()
{
	...
	//	state_zero.segment<3>(0) = SCALE_XI_TRANS *
	//state_zero.segment<3>(0); 	state_zero.segment<3>(3) = SCALE_XI_ROT
	//* state_zero.segment<3>(3); 	state_zero[6] = SCALE_A * state_zero[6];
...
void
EFResidual::fixLinearizationF(EnergyFunctional* ef)
{
	...
	for (int i = 0; i < patternNum; i += 4) {
		...
		rtz = _mm_sub_ps(
		  rtz,
		  _mm_mul_ps(_mm_load_ps(((float*)(J->JIdx)) + i), Jp_delta_x));
		rtz = _mm_sub_ps(
		  rtz,
		  _mm_mul_ps(_mm_load_ps(((float*)(J->JIdx + 1)) + i),
			     Jp_delta_y));
		rtz = _mm_sub_ps(
		  rtz,
		...
		rtz = _mm_sub_ps(
		  rtz,
		  _mm_mul_ps(_mm_load_ps(((float*)(J->JabF + 1)) + i),
			     delta_b));
