...
void EnergyFunctional::accumulateSCF_MT(MatXX& H, VecX& b, bool MT) {
        if (MT) {
                red->reduce(boost::bind(&AccumulatedSCHessianSSE::setZero,
                                        accSSE_bot, nFrames, _1, _2, _3, _4),
                            0, 0, 0);
                red->reduce(
...
void EnergyFunctional::resubstituteFPt(const VecCf& xc,
                                       Mat18f* xAd,
                                       int min,
                                       int max,
                                       Vec10* stats,
...
void EnergyFunctional::orthogonalize(VecX* b, MatXX* H) {
        ...
        std::vector<VecX> ns;
        ns.insert(ns.end(), lastNullspaces_pose.begin(),
                  lastNullspaces_pose.end());
...
void EnergyFunctional::solveSystemF(int iteration,
                                    double lambda,
                                    CalibHessian* HCalib) {
        ...
        if (setting_solverMode & SOLVER_SVD) {
                ...
                x = SVecI.asDiagonal() *
                    HFinalScaled.ldlt().solve(
                        SVecI.asDiagonal() * bFinal_top);  //  SVec.asDiagonal()
                                                           //  * svd.matrixV() *
                                                           //  Ub;
        }
