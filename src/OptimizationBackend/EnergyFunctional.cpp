...
void
EnergyFunctional::setAdjointsF(CalibHessian* Hcalib)
{
	...
	for (int h = 0; h < nFrames; h++)
		for (int t = 0; t < nFrames; t++) {
			...
			AH.topLeftCorner<6, 6>() =
			  -hostToTarget.Adj().transpose();
			AT.topLeftCorner<6, 6>() = Mat66::Identity();

			Vec2f affLL =
			  AffLight::fromToVecExposure(host->ab_exposure,
						      target->ab_exposure,
						      host->aff_g2l_0(),
						      target->aff_g2l_0())
			    .cast<float>();
...
void
EnergyFunctional::setDeltaF(CalibHessian* HCalib)
{
	if (adHTdeltaF != 0)
		delete[] adHTdeltaF;
...
void
EnergyFunctional::accumulateAF_MT(MatXX& H, VecX& b, bool MT)
{
	if (MT) {
		red->reduce(boost::bind(&AccumulatedTopHessianSSE::setZero,
			    ...
			    0);
		red->reduce(
		  boost::bind(&AccumulatedTopHessianSSE::addPointsInternal<0>,
			      accSSE_top_A,
			      &allPoints,
			      this,
			      _1,
...
// accumulates & shifts L.
void
EnergyFunctional::accumulateLF_MT(MatXX& H, VecX& b, bool MT)
{
	if (MT) {
		red->reduce(boost::bind(&AccumulatedTopHessianSSE::setZero,
					accSSE_top_L,
			    ...
			    0);
		red->reduce(
		  boost::bind(&AccumulatedTopHessianSSE::addPointsInternal<1>,
			      accSSE_top_L,
			      &allPoints,
			      this,
			      _1,
...
void
EnergyFunctional::accumulateSCF_MT(MatXX& H, VecX& b, bool MT)
{
	if (MT) {
		red->reduce(boost::bind(&AccumulatedSCHessianSSE::setZero,
					accSSE_bot,
					nFrames,
					_1,
					_2,
			    ...
			    0);
		red->reduce(
		  boost::bind(&AccumulatedSCHessianSSE::addPointsInternal,
			      accSSE_bot,
			      &allPoints,
			      true,
			      _1,
...
void
EnergyFunctional::resubstituteF_MT(VecX x, CalibHessian* HCalib, bool MT)
{
...
void
EnergyFunctional::resubstituteFPt(const VecCf& xc,
				  Mat18f* xAd,
				  int min,
				  int max,
				  Vec10* stats,
	...
	for (int k = min; k < max; k++) {
		...
		for (EFResidual* r : p->residualsAll) {
			if (!r->isActive())
				continue;
			b -=
			  xAd[r->hostIDX * nFrames + r->targetIDX] * r->JpJdF;
		}
...
void
EnergyFunctional::calcLEnergyPt(int min, int max, Vec10* stats, int tid)
{
	...
	for (int i = min; i < max; i++) {
		...
		for (EFResidual* r : p->residualsAll) {
			...
			Mat18f dp =
			  adHTdeltaF[r->hostIDX + nFrames * r->targetIDX];
			...
			for (int i = 0; i + 3 < patternNum; i += 4) {
				// PATTERN: E = (2*res_toZeroF + J*delta) *
				// J*delta.
				__m128 Jdelta = _mm_mul_ps(
				  _mm_load_ps(((float*)(rJ->JIdx)) + i),
				  Jp_delta_x);
				Jdelta = _mm_add_ps(
				  Jdelta,
				  _mm_mul_ps(
				    _mm_load_ps(((float*)(rJ->JIdx + 1)) + i),
				    Jp_delta_y));
				Jdelta = _mm_add_ps(
				  Jdelta,
				  _mm_mul_ps(
				    _mm_load_ps(((float*)(rJ->JabF)) + i),
				    delta_a));
				Jdelta = _mm_add_ps(
				  Jdelta,
				  _mm_mul_ps(
				    _mm_load_ps(((float*)(rJ->JabF + 1)) + i),
				    delta_b));

				__m128 r0 =
				  _mm_load_ps(((float*)&r->res_toZeroF) + i);
				r0 = _mm_add_ps(r0, r0);
				r0 = _mm_add_ps(r0, Jdelta);
				Jdelta = _mm_mul_ps(Jdelta, r0);
				E.updateSSENoShift(Jdelta);
			}
			for (int i = ((patternNum >> 2) << 2); i < patternNum;
			     i++) {
...
double
EnergyFunctional::calcLEnergyF_MT()
{
	assert(EFDeltaValid);
	assert(EFAdjointsValid);
	assert(EFIndicesValid);
	...
	red->reduce(
	  boost::bind(&EnergyFunctional::calcLEnergyPt, this, _1, _2, _3, _4),
	  0,
	  allPoints.size(),
	  50);
...
EFResidual*
EnergyFunctional::insertResidual(PointFrameResidual* r)
{
	EFResidual* efr = new EFResidual(
	  r, r->point->efPoint, r->host->efFrame, r->target->efFrame);
...
EFFrame*
EnergyFunctional::insertFrame(FrameHessian* fh, CalibHessian* Hcalib)
{
	...
	for (EFFrame* fh2 : frames) {
		connectivityMap[(((uint64_t)eff->frameID) << 32) +
				((uint64_t)fh2->frameID)] =
		  Eigen::Vector2i(0, 0);
		if (fh2 != eff)
			connectivityMap[(((uint64_t)fh2->frameID) << 32) +
					((uint64_t)eff->frameID)] =
			  Eigen::Vector2i(0, 0);
	}
...
EFPoint*
EnergyFunctional::insertPoint(PointHessian* ph)
{
...
void
EnergyFunctional::dropResidual(EFResidual* r)
{
	EFPoint* p = r->point;
...
void
EnergyFunctional::marginalizeFrame(EFFrame* fh)
{
	...
	VecX SVec = (HM.diagonal().cwiseAbs() + VecX::Constant(HM.cols(), 10))
		      .cwiseSqrt();
...
void
EnergyFunctional::removePoint(EFPoint* p)
{
	for (EFResidual* r : p->residualsAll)
		dropResidual(r);
...
void
EnergyFunctional::orthogonalize(VecX* b, MatXX* H)
{
	...
	std::vector<VecX> ns;
	ns.insert(
	  ns.end(), lastNullspaces_pose.begin(), lastNullspaces_pose.end());
	ns.insert(
	  ns.end(), lastNullspaces_scale.begin(), lastNullspaces_scale.end());
...
void
EnergyFunctional::solveSystemF(int iteration,
			       double lambda,
			       CalibHessian* HCalib)
{
	...
	if (setting_solverMode & SOLVER_SVD) {
		...
		Eigen::JacobiSVD<MatXX> svd(
		  HFinalScaled, Eigen::ComputeThinU | Eigen::ComputeThinV);
		...
		for (int i = 0; i < Ub.size(); i++) {
			...
			if ((setting_solverMode & SOLVER_SVD_CUT7) &&
			    (i >= Ub.size() - 7)) {
				Ub[i] = 0;
				setZero++;
			}
		...
		VecX SVecI = (HFinal_top.diagonal() +
			      VecX::Constant(HFinal_top.cols(), 10))
			       .cwiseSqrt()
			       .cwiseInverse();
		MatXX HFinalScaled =
		  SVecI.asDiagonal() * HFinal_top * SVecI.asDiagonal();
...
VecX
EnergyFunctional::getStitchedDeltaF() const
{
