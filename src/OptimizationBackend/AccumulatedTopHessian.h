...
class AccumulatedTopHessianSSE
{
      public:
        ...
        inline void setZero(int nFrames,
                            int min = 0,
                            int max = 1,
                            Vec10* stats = 0,
                            int tid = 0)
        ...
        template<int mode>
        void addPoint(EFPoint* p,
                      EnergyFunctional const* const ef,
                      int tid = 0);

        void stitchDoubleMT(IndexThreadReduce<Vec10>* red,
                            MatXX& H,
                            VecX& b,
                            EnergyFunctional const* const EF,
                            bool usePrior,
                            bool MT)
        {
                ...
                if (MT) {
                        ...
                        for (int i = 0; i < NUM_THREADS; i++) {
                                ...
                                Hs[i] = MatXX::Zero(nframes[0] * 8 + CPARS,
                                                    nframes[0] * 8 + CPARS);
                        ...
                        red->reduce(
                          boost::bind(
                            &AccumulatedTopHessianSSE::stitchDoubleInternal,
                            this,
                            Hs,
                            bs,
                            EF,
                        ...
                        H = MatXX::Zero(nframes[0] * 8 + CPARS,
                                        nframes[0] * 8 + CPARS);
                        b = VecX::Zero(nframes[0] * 8 + CPARS);
                        stitchDoubleInternal(&H,
                                             &b,
                                             EF,
                                             usePrior,
                                             0,
      ...
      private:
        void stitchDoubleInternal(MatXX* H,
                                  VecX* b,
                                  EnergyFunctional const* const EF,
                                  bool usePrior,
                                  int min,
