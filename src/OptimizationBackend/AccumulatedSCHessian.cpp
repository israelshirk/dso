...
void
AccumulatedSCHessianSSE::addPoint(EFPoint* p, bool shiftPriorToZero, int tid)
{
        int ngoodres = 0;
...
void
AccumulatedSCHessianSSE::stitchDoubleInternal(MatXX* H,
                                              VecX* b,
        ...
        for (int k = min; k < max; k++) {
                ...
                for (int k = 0; k < nf; k++) {
                        ...
                        H[tid].block<8, 8>(iIdx, iIdx) +=
                          EF->adHost[ijIdx] * accDM *
                          EF->adHost[ikIdx].transpose();
                        H[tid].block<8, 8>(jIdx, kIdx) +=
                          EF->adTarget[ijIdx] * accDM *
...
void
AccumulatedSCHessianSSE::stitchDouble(MatXX& H,
                                      VecX& b,
                                      EnergyFunctional const* const EF,
                                      int tid)
{
        ...
        for (int i = 0; i < nf; i++)
                for (int j = 0; j < nf; j++) {
                        ...
                        H.block<8, CPARS>(jIdx, 0) +=
                          EF->adTarget[ijIdx] * accEM;
                        ...
                        for (int k = 0; k < nf; k++) {
                                ...
                                if (accD[tid][ijkIdx].num == 0)
                                        continue;
                                Mat88 accDM =
                                  accD[tid][ijkIdx].A1m.cast<double>();
