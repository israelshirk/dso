...
double PointFrameResidual::linearize(CalibHessian *HCalib) {
        ...
        float d_d_x, d_d_y;
        {
                ...
                if (!projectPoint(point->u, point->v, point->idepth_zero_scaled,
                                  0, 0, HCalib, PRE_RTll_0, PRE_tTll_0,
                                  drescale, u, v, Ku, Kv, KliP, new_idepth)) {
                        state_NewState = ResState::OOB;
                        return state_energy;
                }
                ...
                d_C_x[3] = HCalib->fxl() * drescale *
                           (PRE_RTll_0(2, 1) * u - PRE_RTll_0(0, 1)) *
                           HCalib->fyli();
                d_C_x[0] = KliP[0] * d_C_x[2];
                d_C_x[1] = KliP[1] * d_C_x[3];

                d_C_y[2] = HCalib->fyl() * drescale *
                           (PRE_RTll_0(2, 0) * v - PRE_RTll_0(1, 0)) *
                           HCalib->fxli();
        ...
        for (int idx = 0; idx < patternNum; idx++) {
                ...
                if (!projectPoint(point->u + patternP[idx][0],
                                  ...
                                  point->idepth_scaled, PRE_KRKiTll, PRE_KtTll,
                                  Ku, Kv)) {
                        state_NewState = ResState::OOB;
                        return state_energy;
                }
...

                {
                        ...
                        wJI2_sum += hw * hw *
                                    (hitColor[1] * hitColor[1] +
                                     hitColor[2] * hitColor[2]);
