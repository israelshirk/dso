...
FullSystem::debugPlotTracking()
{
        ...
        for (FrameHessian* f : frameHessians) {
                ...
                // make images for all frames. will be deleted by the
                // FrameHessian's destructor.
                for (FrameHessian* f2 : frameHessians)
                        if (f2->debugImage == 0)
                                f2->debugImage =
                                  new MinimalImageB3(wG[0], hG[0]);
                ...
                for (FrameHessian* f2 : frameHessians) {
                        ...
                        Vec2 affL = AffLight::fromToVecExposure(f2->ab_exposure,
                                                                f->ab_exposure,
                                                                f2->aff_g2l(),
                                                                f->aff_g2l());
...
void
FullSystem::debugPlot(std::string name)
{
        if (disableAllDisplay)
                return;
        ...
        if ((int)(freeDebugParam5 + 0.5f) == 7 || (debugSaveImages && false)) {
                ...
                for (unsigned int f = 0; f < frameHessians.size(); f++) {
                        ...
                        for (PointHessian* ph :
                             frameHessians[f]->pointHessiansMarginalized)
                                if (ph != 0)
                                        allID.push_back(ph->idepth_scaled);

                        for (PointHessian* ph :
                             frameHessians[f]->pointHessiansOut)
                                if (ph != 0)
                                        allID.push_back(ph->idepth_scaled);
                }
        ...
        for (unsigned int f = 0; f < frameHessians.size(); f++) {
                ...
                if ((int)(freeDebugParam5 + 0.5f) == 0) {
                        ...
                        for (PointHessian* ph :
                             frameHessians[f]->pointHessiansMarginalized)
                                img->setPixelCirc(
                                  ph->u + 0.5f, ph->v + 0.5f, Vec3b(0, 0, 0));
                        ...
                        for (ImmaturePoint* ph :
                             frameHessians[f]->immaturePoints) {
                                if (ph == 0)
                                        continue;
                             ...
                             frameHessians[f]->immaturePoints) {
                                ...
                                if (ph->lastTraceStatus ==
                                    ImmaturePointStatus::IPS_UNINITIALIZED)
                                        continue;
                                float d =
                                  freeDebugParam1 * (sqrtf(ph->quality) - 1);
                                if (d < 0)
                                        d = 0;
                                if (d > 1)
                                        d = 1;
                        ...
                        for (PointHessian* ph :
                             frameHessians[f]->pointHessians) {
                                img->setPixelCirc(
                                  ph->u + 0.5f,
                                  ph->v + 0.5f,
                                  makeJet3B((ph->idepth_scaled - minID) /
                                            ((maxID - minID))));
                        }
                        for (PointHessian* ph :
                             frameHessians[f]->pointHessiansMarginalized) {
        ...
        if ((debugSaveImages && false)) {
                for (unsigned int f = 0; f < frameHessians.size(); f++) {
                        ...
                        snprintf(buf,
                                 1000,
                                 "images_out/kf_%05d_%05d_%02d.png",
