        ...
        for (PointHessian* ph : f->pointHessians) {
            ...
            if (ph->status == PointHessian::ACTIVE ||
                ...
                f->debugImage->setPixel9(
                  ph->u + 0.5, ph->v + 0.5, makeRainbow3B(ph->idepth_scaled));
            }
        }
    ...
    for (unsigned int f = 0; f < frameHessians.size(); f++) {
        ...
        if ((int)(freeDebugParam5 + 0.5f) == 0) {
            for (PointHessian* ph : frameHessians[f]->pointHessians) {
                ...
                img->setPixelCirc(
                  ph->u + 0.5f, ph->v + 0.5f, makeRainbow3B(ph->idepth_scaled));
            }
            for (PointHessian* ph :
            ...
            for (PointHessian* ph : frameHessians[f]->pointHessiansOut)
                img->setPixelCirc(
                  ph->u + 0.5f, ph->v + 0.5f, Vec3b(255, 255, 255));
            ...
            for (ImmaturePoint* ph : frameHessians[f]->immaturePoints) {
                ...
                if (d > 1)
                    d = 1;
                img->setPixelCirc(
                  ph->u + 0.5f, ph->v + 0.5f, Vec3b(0, d * 255, (1 - d) * 255));
            }
            ...
            for (PointHessian* ph : frameHessians[f]->pointHessians) {
                ...
                if (ph->my_type == 0)
                    img->setPixelCirc(
                      ph->u + 0.5f, ph->v + 0.5f, Vec3b(255, 0, 255));
                if (ph->my_type == 1)
                    img->setPixelCirc(
                      ph->u + 0.5f, ph->v + 0.5f, Vec3b(255, 0, 0));
                if (ph->my_type == 2)
                ...
                if (ph->my_type == 3)
                    img->setPixelCirc(
                      ph->u + 0.5f, ph->v + 0.5f, Vec3b(0, 255, 255));
            }
            for (PointHessian* ph :
                 frameHessians[f]->pointHessiansMarginalized) {
                if (ph == 0)
