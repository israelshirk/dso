...
void
FullSystem::flagFramesForMarginalization(FrameHessian* newFH)
{
	if (setting_minFrameAge > setting_maxFrames) {
		for (int i = setting_maxFrames; i < (int)frameHessians.size();
		     i++) {
	...
	for (int i = 0; i < (int)frameHessians.size(); i++) {
		...
		Vec2 refToFh =
		  AffLight::fromToVecExposure(frameHessians.back()->ab_exposure,
					      fh->ab_exposure,
		...
		if ((in < setting_minPointsRemaining * (in + out) ||
		     fabs(logf((float)refToFh[0])) >
		       setting_maxLogAffFacInWindow) &&
		    ((int)frameHessians.size()) - flagged > setting_minFrames) {
			//			printf("MARGINALIZE frame %d, as
			//only %'d/%'d points remaining (%'d %'d %'d %'d).
			...
			//			printf("May Keep frame %d, as
			//%'d/%'d points remaining (%'d %'d %'d %'d). VisInLast
			//%'d / %'d. traces %d, activated %d!\n",
			//					fh->frameID, in,
			//in+out,
			//					(int)fh->pointHessians.size(),
			//(int)fh->immaturePoints.size(),
	...
	if ((int)frameHessians.size() - flagged >= setting_maxFrames) {
		...
		for (FrameHessian* fh : frameHessians) {
			if (fh->frameID >
			      latest->frameID - setting_minFrameAge ||
			    fh->frameID == 0)
				continue;
			...
			distScore *=
			  -sqrtf(fh->targetPrecalc.back().distanceLL);
		...
		//		printf("MARGINALIZE frame %d, as it is the closest
		//(score %.2f)!\n",
...
void
FullSystem::marginalizeFrame(FrameHessian* frame)
{
	...
	for (FrameHessian* fh : frameHessians) {
		...
		for (PointHessian* ph : fh->pointHessians) {
			for (unsigned int i = 0; i < ph->residuals.size();
			     i++) {
				PointFrameResidual* r = ph->residuals[i];
				if (r->target == frame) {
					if (ph->lastResiduals[0].first == r)
						ph->lastResiduals[0].first = 0;
					else if (ph->lastResiduals[1].first ==
						 r)
