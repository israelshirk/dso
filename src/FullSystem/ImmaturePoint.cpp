...
namespace dso {
ImmaturePoint::ImmaturePoint(int u_,
			     int v_,
			     FrameHessian* host_,
			     float type,
			     CalibHessian* HCalib)
	...
	for (int idx = 0; idx < patternNum; idx++) {
		...
		Vec3f ptc = getInterpolatedElement33BiLin(host->dI, u + dx,
							  v + dy, wG[0]);
...
ImmaturePointStatus ImmaturePoint::traceOn(FrameHessian* frame,
	...
	if (debugPrint)
		printf(
		    "trace pt (%.1f %.1f) from frame %d to %d. Range %f -> %f. "
		    "t %f %f %f!\n",
		    u, v, host->shell->id, frame->shell->id, idepth_min,
	...
	if (!(uMin > 4 && vMin > 4 && uMin < wG[0] - 5 && vMin < hG[0] - 5)) {
		if (debugPrint)
			printf("OOB uMin %f %f - %f %f %f (id %f-%f)!\n", u, v,
			       uMin, vMin, ptpMin[2], idepth_min, idepth_max);
	...
	if (std::isfinite(idepth_max)) {
		...
		// ============== check their distance. everything below 2px is
		// OK (-> skip). ===================
		dist = (uMin - uMax) * (uMin - uMax) +
		       (vMin - vMax) * (vMin - vMax);
		dist = sqrtf(dist);
	...
	if (debugPrint)
		printf(
		    "trace pt (%.1f %.1f) from frame %d to %d. Range %f (%.1f "
		    "%.1f) -> %f (%.1f %.1f)! ErrorInPixel %.1f!\n",
		    u, v, host->shell->id, frame->shell->id, idepth_min, uMin,
		    vMin, idepth_max, uMax, vMax, errorInPixel);
	...
	for (int i = 0; i < numSteps; i++) {
		...
		for (int idx = 0; idx < patternNum; idx++) {
			...
			float residual =
			    hitColor -
			    (float)(hostToFrame_affine[0] * color[idx] +
	...
	for (int it = 0; it < setting_trace_GNIterations; it++) {
		...
		for (int idx = 0; idx < patternNum; idx++) {
			...
			float residual =
			    hitColor[0] - (hostToFrame_affine[0] * color[idx] +
					   hostToFrame_affine[1]);
			...
			energy += weights[idx] * weights[idx] * hw * residual *
				  residual * (2 - hw);
		}
		...
		if (energy > bestEnergy) {
			...
			if (debugPrint)
				printf(
				    "GN BACK %d: E %f, H %f, b %f. id-step %f. "
				    "UV %f %f -> %f %f.\n",
				    it, energy, H, b, stepBack, uBak, vBak,
				    bestU, bestV);
		} else {
			gnStepsGood++;
			...
			if (debugPrint)
				printf(
				    "GN step %d: E %f, H %f, b %f. id-step %f. "
				    "UV %f %f -> %f %f.\n",
				    it, energy, H, b, step, uBak, vBak, bestU,
				    bestV);
		}
...
float ImmaturePoint::getdPixdd(CalibHessian* HCalib,
	...
	FrameFramePrecalc* precalc =
	    &(host->targetPrecalc[tmpRes->target->idx]);
...
float ImmaturePoint::calcResidual(CalibHessian* HCalib,
	...
	for (int idx = 0; idx < patternNum; idx++) {
		...
		// (getInterpolatedElement13BiCub(tmpRes->target->I, Ku, Kv,
		// wG[0]));

		float residual =
		    hitColor[0] - (affLL[0] * color[idx] + affLL[1]);

		float hw = fabsf(residual) < setting_huberTH
			       ? 1
			       : setting_huberTH / fabsf(residual);
		energyLeft += weights[idx] * weights[idx] * hw * residual *
			      residual * (2 - hw);
	}
...
double ImmaturePoint::linearizeResidual(CalibHessian* HCalib,
	...
	for (int idx = 0; idx < patternNum; idx++) {
		...
		float d_idepth = derive_idepth(PRE_tTll, u, v, dx, dy, dxInterp,
					       dyInterp, drescale);
