 ...
 */
ImmaturePointStatus
ImmaturePoint::traceOn(FrameHessian* frame,
		       ...
		       bool debugPrint)
{
	...
	if (debugPrint)
		printf("trace pt (%.1f %.1f) from frame %d to %d. Range %f -> "
		       "%f. t %f %f %f!\n",
		       u,
		       v,
		       host->shell->id,
	...
	if (std::isfinite(idepth_max)) {
		...
		// OK (-> skip). ===================
		dist =
		  (uMin - uMax) * (uMin - uMax) + (vMin - vMax) * (vMin - vMax);
		dist = sqrtf(dist);
	...
	for (int it = 0; it < setting_trace_GNIterations; it++) {
		...
		if (energy > bestEnergy) {
			...
			if (debugPrint)
				printf("GN BACK %d: E %f, H %f, b %f. id-step "
				       "%f. UV %f %f -> %f %f.\n",
				       it,
				       energy,
				       H,
			...
			if (debugPrint)
				printf("GN step %d: E %f, H %f, b %f. id-step "
				       "%f. UV %f %f -> %f %f.\n",
				       it,
				       energy,
				       H,
...
float
ImmaturePoint::getdPixdd(CalibHessian* HCalib,
...
float
ImmaturePoint::calcResidual(CalibHessian* HCalib,
...
double
ImmaturePoint::linearizeResidual(CalibHessian* HCalib,
				 const float outlierTHSlack,
				 ImmaturePointTemporaryResidual* tmpRes,
				 float& Hdd,
				 float& bd,
				 float idepth)
{
	...
	for (int idx = 0; idx < patternNum; idx++) {
		...
		float d_idepth = derive_idepth(
		  PRE_tTll, u, v, dx, dy, dxInterp, dyInterp, drescale);
