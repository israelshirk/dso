		...
		if (fixLinearization) {
			...
			if (r->efResidual->isActive()) {
				if (r->isNew) {
					...
					Vec3f ptp =
					    ptp_inf +
					    r->host->targetPrecalc[r->target
								       ->idx]
						    .PRE_KtTll *
						p->idepth_scaled;  // projected
								   // point with
								   // real
								   // depth.
					float relBS =
...
void FullSystem::applyRes_Reductor(bool copyJacobians,
				   int min,
				   int max,
				   Vec10* stats,
				   int tid) {
...
void FullSystem::printOptRes(const Vec3& res,
			     double resL,
			     double resM,
			     double resPrior,
			     double LExact,
...
float FullSystem::optimize(int mnumOptIts) {
	...
	for (int iteration = 0; iteration < mnumOptIts; iteration++) {
		...
		if (std::isfinite(incDirChange) &&
			...
			stepsize = sqrtf(sqrtf(newStepsize * stepsize *
					       stepsize * stepsize));
			if (stepsize > 2)
				stepsize = 2;
			if (stepsize < 0.25)
		...
		bool canbreak = doStepFromBackup(stepsize, stepsize, stepsize,
						 stepsize, stepsize);
		...
		if (!setting_debugout_runquiet) {
			printf("%s %d (L %.2f, dir %.2f, ss %.1f): \t",
				...
				lastEnergy[0] + lastEnergy[1] + lastEnergyL +
				   ...
				   ? "ACCEPT"
				   : "REJECT",
			       iteration, log10(lambda), incDirChange,
			       stepsize);
		...
		if (setting_forceAceptStep ||
			...
			if (multiThreading)
				...
				applyRes_Reductor(true, 0,
						  activeResiduals.size(), 0, 0);
