...
void
FullSystem::linearizeAll_Reductor(bool fixLinearization,
			...
			if (r->efResidual->isActive()) {
				if (r->isNew) {
					...
					Vec3f ptp =
					  ...
					  r->host->targetPrecalc[r->target->idx]
					      .PRE_KtTll *
					    p->idepth_scaled; // projected point
							      // with real
							      // depth.
					float relBS =
					  0.01 *
					  ((ptp_inf.head<2>() / ptp_inf[2]) -
					   (ptp.head<2>() / ptp[2]))
					    .norm(); // 0.01 = one pixel.
...
void
FullSystem::applyRes_Reductor(bool copyJacobians,
			      int min,
			      int max,
			      Vec10* stats,
			      int tid)
...
FullSystem::setNewFrameEnergyTH()
{
	...
	std::nth_element(
	  allResVec.begin(), allResVec.begin() + nthIdx, allResVec.end());
	...
	newFrame->frameEnergyTH =
	  newFrame->frameEnergyTH * newFrame->frameEnergyTH;
	newFrame->frameEnergyTH *=
...
FullSystem::linearizeAll(bool fixLinearization)
{
	...
	if (multiThreading) {
		treadReduce.reduce(
		  boost::bind(&FullSystem::linearizeAll_Reductor,
			      this,
			      fixLinearization,
			      toRemove,
			      _1,
		...
		Vec10 stats;
		linearizeAll_Reductor(fixLinearization,
				      toRemove,
				      0,
				      activeResiduals.size(),
				      &stats,
...
bool
FullSystem::doStepFromBackup(float stepfacC,
			     float stepfacT,
			     float stepfacR,
			     float stepfacA,
			     float stepfacD)
{
	...
	if (!setting_debugout_runquiet)
		printf("STEPS: A %.1f; B %.1f; R %.1f; T %.1f. \t",
		       ...
		       sqrtf(sumT) * sumNID /
			 (0.00005 * setting_thOptIterations));
...
void
FullSystem::backupState(bool backupLastStep)
{
	if (setting_solverMode & SOLVER_MOMENTUM) {
		if (backupLastStep) {
...
void
FullSystem::printOptRes(const Vec3& res,
			double resL,
			double resM,
			double resPrior,
			double LExact,
...
float
FullSystem::optimize(int mnumOptIts)
{
	...
	if (multiThreading)
		treadReduce.reduce(
		  boost::bind(
		    &FullSystem::applyRes_Reductor, this, true, _1, _2, _3, _4),
		  0,
		  activeResiduals.size(),
		  50);
	else
	...
	for (int iteration = 0; iteration < mnumOptIts; iteration++) {
		...
		if (std::isfinite(incDirChange) &&
			...
			stepsize = sqrtf(
			  sqrtf(newStepsize * stepsize * stepsize * stepsize));
			if (stepsize > 2)
				stepsize = 2;
			if (stepsize < 0.25)
				stepsize = 0.25;
		...
		bool canbreak = doStepFromBackup(
		...
		if (setting_forceAceptStep ||
			...
			if (multiThreading)
				treadReduce.reduce(
				  boost::bind(&FullSystem::applyRes_Reductor,
					      this,
					      true,
					      _1,
					      _2,
				...
				applyRes_Reductor(
				  true, 0, activeResiduals.size(), 0, 0);
...
void
FullSystem::solveSystem(int iteration, double lambda)
{
...
std::vector<VecX>
FullSystem::getNullspaces(std::vector<VecX>& nullspaces_pose,
			  ...
			  std::vector<VecX>& nullspaces_affB)
{
	...
	for (int i = 0; i < 2; i++) {
		...
		for (FrameHessian* fh : frameHessians) {
			...
			nullspace_x0[CPARS + fh->idx * 8 + 6] *=
			  SCALE_A_INVERSE;
			nullspace_x0[CPARS + fh->idx * 8 + 7] *=
			  SCALE_B_INVERSE;
		}
		nullspaces_x0_pre.push_back(nullspace_x0);
		if (i == 0)
