...
PointHessian* FullSystem::optimizeImmaturePoint(
    ...
    ImmaturePointTemporaryResidual* residuals) {
        ...
        if (!std::isfinite(lastEnergy) || lastHdd < setting_minIdepthH_act) {
                if (print)
                        printf(
                            "OptPoint: Not well-constrained (%d res, H=%.1f). "
                            "E=%f. SKIP!\n",
        ...
        if (print)
                printf(
                    "Activate point. %d residuals. H=%f. Initial Energy: %f. "
                    "Initial Id=%f\n",
             ...
             iteration++) {
                ...
                if (!std::isfinite(lastEnergy) ||
                    newHdd < setting_minIdepthH_act) {
                        if (print)
                                printf(
                                    "OptPoint: Not well-constrained (%d res, "
                                    "H=%.1f). E=%f. SKIP!\n",
        ...
        if (!std::isfinite(currentIdepth)) {
                printf(
                    "MAJOR ERROR! point idepth is nan after initialization "
                    "(%f).\n",
                    currentIdepth);
