...
PointHessian*
FullSystem::optimizeImmaturePoint(ImmaturePoint* point,
				  int minObs,
				  ImmaturePointTemporaryResidual* residuals)
{
	...
	for (FrameHessian* fh : frameHessians) {
		if (fh != point->host) {
			residuals[nres].state_NewEnergy =
			  residuals[nres].state_energy = 0;
	...
	for (int i = 0; i < nres; i++) {
		lastEnergy += point->linearizeResidual(
		  &Hcalib, 1000, residuals + i, lastHdd, lastbd, currentIdepth);
	...
	if (!std::isfinite(lastEnergy) || lastHdd < setting_minIdepthH_act) {
		if (print)
			printf("OptPoint: Not well-constrained (%d res, "
			       "H=%.1f). E=%f. SKIP!\n",
			       nres,
			       lastHdd,
			       lastEnergy);
	...
	if (print)
		printf("Activate point. %d residuals. H=%f. Initial Energy: "
		       "%f. Initial Id=%f\n",
		       nres,
		       lastHdd,
		       lastEnergy,
	...
	for (int iteration = 0; iteration < setting_GNItsOnPointActivation;
		...
		for (int i = 0; i < nres; i++)
			newEnergy += point->linearizeResidual(
			  &Hcalib, 1, residuals + i, newHdd, newbd, newIdepth);

		if (!std::isfinite(lastEnergy) ||
		    newHdd < setting_minIdepthH_act) {
			if (print)
				printf("OptPoint: Not well-constrained (%d "
				       "res, H=%.1f). E=%f. SKIP!\n",
				       nres,
				       newHdd,
				       lastEnergy);
		...
		if (print)
			printf("%s %d (L %.2f) %s: %f -> %f (idepth %f)!\n",
			       (true || newEnergy < lastEnergy) ? "ACCEPT"
								: "REJECT",
			       iteration,
			       log10(lambda),
			       "",
		...
		if (newEnergy < lastEnergy) {
			...
			for (int i = 0; i < nres; i++) {
				residuals[i].state_state =
				  residuals[i].state_NewState;
				residuals[i].state_energy =
				  residuals[i].state_NewEnergy;
			}
	...
	if (!std::isfinite(currentIdepth)) {
		printf("MAJOR ERROR! point idepth is nan after initialization "
		       "(%f).\n",
		       currentIdepth);
		return (PointHessian*)((long)(-1)); // yeah I'm like 99% sure
						    // this is OK on 32bit
						    // systems.
	}
