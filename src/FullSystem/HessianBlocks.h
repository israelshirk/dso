...
struct FrameFramePrecalc {
	...
	void set(FrameHessian* host, FrameHessian* target,
		 CalibHessian* HCalib);
};
...
struct FrameHessian {
	...
	Eigen::Vector3f* dIp[PYR_LEVELS];  // coarse tracking / coarse
	...
	std::vector<PointHessian*>
	    pointHessians;  // contains all ACTIVE points.
	...
	inline void setState(const Vec10& state) {
		this->state = state;
		state_scaled.segment<3>(0) =
		    SCALE_XI_TRANS * state.segment<3>(0);
		...
		PRE_worldToCam =
		    SE3::exp(w2c_leftEps()) * get_worldToCam_evalPT();
	...
	inline void setStateScaled(const Vec10& state_scaled) {
		...
		state.segment<3>(3) =
		    SCALE_XI_ROT_INVERSE * state_scaled.segment<3>(3);
	...
	inline void setEvalPT(const SE3& worldToCam_evalPT,
			      const Vec10& state) {
	...
	inline Vec10 getPrior() {
		...
		if (frameID == 0) {
			...
			p.segment<3>(3) =
			    Vec3::Constant(setting_initialRotPrior);
...
struct PointHessian {
	...
	float weights[MAX_RES_PER_POINT];  // host-weights for respective
					   // residuals.
	...
	std::vector<PointFrameResidual*> residuals;  // only contains good
						     // residuals (not OOB and
						     // not OUTLIER). Arbitrary
						     // order.
	std::pair<PointFrameResidual*, ResState>
	    lastResiduals[2];  // contains information about residuals to the
			       // last two (!) frames. ([0] = latest, [1] = the
			       // one before).
