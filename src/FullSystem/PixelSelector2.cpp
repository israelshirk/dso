...
int
computeHistQuantil(int* hist, float below)
{
...
void
PixelSelector::makeHists(const FrameHessian* const fh)
{
        gradHistFrame = fh;
...
int
PixelSelector::makeMaps(const FrameHessian* const fh,
                        float* map_out,
                        float density,
                        int recursionsLeft,
                        bool plot,
                        float thFactor)
{
        ...
        //	else
        {
                ...
                float K =
                  numHave * (currentPotential + 1) * (currentPotential + 1);
                idealPotential = sqrtf(K / numWant) - 1; // round down.
                if (idealPotential < 1)
                        idealPotential = 1;

                if (recursionsLeft > 0 && quotia > 1.25 &&
                    currentPotential > 1) {
                        ...
                        //		printf("PixelSelector: have %.2f%%, need
                        //%.2f%%. RESAMPLE with pot %d -> %d.\n",
...
Eigen::Vector3i
PixelSelector::select(const FrameHessian* const fh,
                      float* map_out,
                      int pot,
                      float thFactor)
{
        ...
        const Vec2f directions[16] = {
                Vec2f(0, 1.0000),       Vec2f(0.3827, 0.9239),
                Vec2f(0.1951, 0.9808),  Vec2f(0.9239, 0.3827),
                Vec2f(0.7071, 0.7071),  Vec2f(0.3827, -0.9239),
                Vec2f(0.8315, 0.5556),  Vec2f(0.8315, -0.5556),
                Vec2f(0.5556, -0.8315), Vec2f(0.9808, 0.1951),
                Vec2f(0.9239, -0.3827), Vec2f(0.7071, -0.7071),
                Vec2f(0.5556, 0.8315),  Vec2f(0.9808, -0.1951),
                Vec2f(1.0000, 0.0000),  Vec2f(0.1951, -0.9808)
        };
                ...
                for (int x4 = 0; x4 < w; x4 += (4 * pot)) {
                        ...
                        for (int y3 = 0; y3 < my3; y3 += (2 * pot))
                                for (int x3 = 0; x3 < mx3; x3 += (2 * pot)) {
                                        ...
                                        float bestVal3 = 0;
                                        Vec2f dir3 =
                                          directions[randomPattern[n2] & 0xF];
                                                                                  ...
                                                                                  fabsf((
                                                                                    float)(ag0d
                                                                                             .dot(
                                                                                               dir2)));
                                                                                if (
                                                                                  !setting_selectDirectionDistribution)
                                                                                        dirNorm =
                                                                                          ag0;
