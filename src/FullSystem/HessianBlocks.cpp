...
void
FrameHessian::setStateZero(const Vec10& state_zero)
{
...
void
FrameHessian::makeImages(float* color, CalibHessian* HCalib)
{
	...
	for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
				...
				for (int x = 0; x < wl; x++) {
					dI_l[x + y * wl][0] =
					  ...
					  (dI_lm[2 * x + 2 * y * wlm1][0] +
					   ...
					   dI_lm[2 * x + 2 * y * wlm1 + wlm1]
						[0] +
					   dI_lm[2 * x + 1 + 2 * y * wlm1 +
						 wlm1][0]);
				}
		}
		...
		for (int idx = wl; idx < wl * (hl - 1); idx++) {
			...
			float dy =
			  0.5f * (dI_l[idx + wl][0] - dI_l[idx - wl][0]);
			...
			if (setting_gammaWeightsPixelSelect == 1 &&
			    HCalib != 0) {
				float gw =
				  HCalib->getBGradOnly((float)(dI_l[idx][0]));
				dabs_l[idx] *= gw * gw; // convert to gradient
							// of original color
							// space (before
							// removing response).
			}
...
void
FrameFramePrecalc::set(FrameHessian* host,
		       FrameHessian* target,
		       CalibHessian* HCalib)
{
	this->host = host;
