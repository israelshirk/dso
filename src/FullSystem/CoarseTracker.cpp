...
CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians)
{
    ...
    for (int lvl = 0; lvl < 2; lvl++) {
        ...
        for (int it = 0; it < numIts; it++) {
            ...
            memcpy(
              weightSumsl_bak, weightSumsl, w[lvl] * h[lvl] * sizeof(float));
                         ...
                         AffLight aff_g2l)
{
    ...
    for (int i = 0; i < n; i += 4) {
        ...
        acc.updateSSE_eighted(
          ...
          _mm_sub_ps(
            zero,
            _mm_mul_ps(id, _mm_add_ps(_mm_mul_ps(u, dx), _mm_mul_ps(v, dy)))),
          _mm_sub_ps(
            zero,
                       ...
                       float cutoffTH)
{
    ...
    Vec2f affLL =
      AffLight::fromToVecExposure(
        lastRef->ab_exposure, newFrame->ab_exposure, lastRef_aff_g2l, aff_g2l)
        .cast<float>();
