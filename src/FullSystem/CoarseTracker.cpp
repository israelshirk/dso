...
T*
allocAligned(int size, std::vector<T*>& rawPtrVec)
{
  ...
  : lastRef_aff_g2l(0, 0)
{
	...
	for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
		...
		weightSums_bak[lvl] =
		  allocAligned<4, float>(wl * hl, ptrToDelete);
...
void
CoarseTracker::makeK(CalibHessian* HCalib)
{
	w[0] = wG[0];
	h[0] = hG[0];
...
void
CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians)
{
	...
	for (FrameHessian* fh : frameHessians) {
		for (PointHessian* ph : fh->pointHessians) {
			if (ph->lastResiduals[0].first != 0 &&
				...
				float weight =
				  sqrtf(1e-3 / (ph->efPoint->HdiF + 1e-12));
	...
	for (int lvl = 1; lvl < pyrLevelsUsed; lvl++) {
		...
		for (int y = 0; y < hl; y++)
			for (int x = 0; x < wl; x++) {
				...
				idepth_l[x + y * wl] =
				  idepth_lm[bidx] + idepth_lm[bidx + 1] +
				  idepth_lm[bidx + wlm1] +
	...
	for (int lvl = 0; lvl < 2; lvl++) {
		...
		for (int it = 0; it < numIts; it++) {
			...
			memcpy(weightSumsl_bak,
			       weightSumsl,
			       w[lvl] * h[lvl] * sizeof(float));
			float* idepthl =
			  idepth[lvl]; // dotnt need to make a temp copy of
				       // depth, since I only read values with
				       // weightSumsl>0, and write ones with
				       // weightSumsl<=0.
	...
	for (int lvl = 2; lvl < pyrLevelsUsed; lvl++) {
		...
		memcpy(weightSumsl_bak,
		       weightSumsl,
		       w[lvl] * h[lvl] * sizeof(float));
		float* idepthl = idepth[lvl]; // dotnt need to make a temp copy
					      // of depth, since I only read
					      // values with weightSumsl>0, and
		...
		for (int y = 2; y < hl - 2; y++)
			for (int x = 2; x < wl - 2; x++) {
				...
				if (weightSumsl[i] > 0) {
					...
					if (!std::isfinite(lpc_color[lpc_n]) ||
					    !(idepthl[i] > 0)) {
						idepthl[i] = -1;
						continue; // just skip if
							  // something is wrong.
					}
					lpc_n++;
				} else
...
void
CoarseTracker::calcGSSSE(int lvl,
			 Mat88& H_out,
			 Vec8& b_out,
			 const SE3& refToNew,
			 AffLight aff_g2l)
{
	...
	__m128 a =
	  _mm_set1_ps((float)(AffLight::fromToVecExposure(lastRef->ab_exposure,
	...
	for (int i = 0; i < n; i += 4) {
		...
		acc.updateSSE_eighted(
		  ...
		  _mm_sub_ps(
		    zero,
		    _mm_mul_ps(
		      id, _mm_add_ps(_mm_mul_ps(u, dx), _mm_mul_ps(v, dy)))),
		  _mm_sub_ps(
		    zero,
		    _mm_add_ps(
		  ...
		  _mm_mul_ps(
		    a, _mm_sub_ps(b0, _mm_load_ps(buf_warped_refColor + i))),
		  minusOne,
		       ...
		       float cutoffTH)
{
	...
	for (int i = 0; i < nl; i++) {
		...
		if (fabs(residual) > cutoffTH) {
			if (debugPlot)
				resImage->setPixel4(
				  lpc_u[i], lpc_v[i], Vec3b(0, 0, 255));
			E += maxEnergy;
			numTermsInE++;
			numSaturated++;
		} else {
			if (debugPlot)
				resImage->setPixel4(lpc_u[i],
						    lpc_v[i],
						    Vec3b(residual + 128,
							  residual + 128,
							  residual + 128));
...
void
CoarseTracker::setCoarseTrackingRef(std::vector<FrameHessian*> frameHessians)
{
...
bool
CoarseTracker::trackNewestCoarse(FrameHessian* newFrameHessian,
				 SE3& lastToNew_out,
	...
	for (int lvl = coarsestLvl; lvl >= 0; lvl--) {
		...
		while (resOld[5] > 0.6 && levelCutoffRepeat < 50) {
			...
			if (!setting_debugout_runquiet)
				printf(
				  "INCREASING cutoff to %f (ratio is %f)!\n",
				  setting_coarseCutoffTH * levelCutoffRepeat,
				  resOld[5]);
		}
		...
		if (debugPrint) {
			Vec2f relAff =
			  AffLight::fromToVecExposure(lastRef->ab_exposure,
						      newFrame->ab_exposure,
						      lastRef_aff_g2l,
						      aff_g2l_current)
			    .cast<float>();
			printf("lvl%d, it %d (l=%f / %f) %s: %.3f->%.3f (%d -> "
			       "%d) (|inc| = %f)! \t",
			       lvl,
			       -1,
			       lambda,
		...
		for (int iteration = 0; iteration < maxIterations[lvl];
		     iteration++) {
			Mat88 Hl = H;
			for (int i = 0; i < 8; i++)
				Hl(i, i) *= (1 + lambda);
			...
			{
				inc.head<7>() =
				  Hl.topLeftCorner<7, 7>().ldlt().solve(
				    -b.head<7>());
				inc.tail<1>().setZero();
			}
			if (setting_affineOptModeA < 0 &&
			    !(setting_affineOptModeB < 0)) // fix a
			{
				...
				bStitch[6] = bStitch[7];
				Vec7 incStitch =
				  HlStitch.topLeftCorner<7, 7>().ldlt().solve(
				    -bStitch.head<7>());
				inc.setZero();
			...
			if (lambda < lambdaExtrapolationLimit)
				extrapFac =
				  sqrt(sqrt(lambdaExtrapolationLimit / lambda));
			inc *= extrapFac;
...
void
CoarseTracker::debugPlotIDepthMap(float* minID_pt,
				  float* maxID_pt,
				  std::vector<IOWrap::Output3DWrapper*>& wraps)
{
...

	{
		...
		if (minID_pt != 0 && maxID_pt != 0) {
			if (*minID_pt < 0 || *maxID_pt < 0) {
				...
				*minID_pt = minID;
				...
				// slowly adapt: change by maximum 10% of old
				// span.
		...
		if (debugSaveImages) {
			...
			snprintf(buf,
				 1000,
				 "images_out/predicted_%05d_%05d.png",
				 lastRef->shell->id,
				 refFrameID);
...
void
CoarseTracker::debugPlotIDepthMapFloat(
  std::vector<IOWrap::Output3DWrapper*>& wraps)
{
	if (w[1] == 0)
		return;
...
void
CoarseDistanceMap::makeDistanceMap(std::vector<FrameHessian*> frameHessians,
				   FrameHessian* frame)
{
	int w1 = w[1];
	int h1 = h[1];
...
void
CoarseDistanceMap::makeInlierVotes(std::vector<FrameHessian*> frameHessians)
{
}

void
CoarseDistanceMap::growDistBFS(int bfsNum)
{
	assert(w[0] != 0);
	int w1 = w[1], h1 = h[1];
	for (int k = 1; k < 40; k++) {
...
void
CoarseDistanceMap::addIntoDistFinal(int u, int v)
{
	if (w[0] == 0)
		return;
...
void
CoarseDistanceMap::makeK(CalibHessian* HCalib)
{
	w[0] = wG[0];
	h[0] = hG[0];
