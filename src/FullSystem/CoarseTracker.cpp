                ...
                for (int it = 0; it < numIts; it++) {
                        ...
                        float* idepthl =
                            idepth[lvl];  // dotnt need to make a temp copy of
                                          // depth, since I only read values
                                          // with weightSumsl>0, and write ones
                                          // with weightSumsl<=0.
...
void CoarseTracker::calcGSSSE(int lvl,
                              Mat88& H_out,
                              Vec8& b_out,
                              const SE3& refToNew,
                              AffLight aff_g2l) {
        ...
        for (int i = 0; i < n; i += 4) {
                ...
                acc.updateSSE_eighted(
                    ...
                    _mm_sub_ps(zero,
                               _mm_mul_ps(id, _mm_add_ps(_mm_mul_ps(u, dx),
                                                         _mm_mul_ps(v, dy)))),
                    _mm_sub_ps(
                        zero,
                        _mm_add_ps(
                    ...
                    _mm_mul_ps(a, _mm_sub_ps(b0, _mm_load_ps(
                                                     buf_warped_refColor + i))),
...
Vec6 CoarseTracker::calcRes(int lvl,
                            const SE3& refToNew,
                            AffLight aff_g2l,
                            float cutoffTH) {
        float E = 0;
        ...
        for (int i = 0; i < nl; i++) {
                ...
                if (fabs(residual) > cutoffTH) {
                        ...
                        if (debugPlot)
                                resImage->setPixel4(
                                    lpc_u[i], lpc_v[i],
                                    Vec3b(residual + 128, residual + 128,
                                          residual + 128));
                ...
                if (debugPrint) {
                        Vec2f relAff =
                            AffLight::fromToVecExposure(
                                ...
                                .cast<float>();
                        printf(
                            "lvl%d, it %d (l=%f / %f) %s: %.3f->%.3f (%d -> "
                            "%d) (|inc| = %f)! \t",
                            lvl, -1, lambda, 1.0f, "INITIA", 0.0f,
                ...
                for (int iteration = 0; iteration < maxIterations[lvl];
                     iteration++) {
                        ...
                        if (lambda < lambdaExtrapolationLimit)
                                extrapFac = sqrt(
                                    sqrt(lambdaExtrapolationLimit / lambda));
                        inc *= extrapFac;
                        ...
                        if (debugPrint) {
                                ...
                                printf(
                                    ...
                                    resOld[0] / resOld[1],
                                    resNew[0] / resNew[1], (int)resOld[1],
