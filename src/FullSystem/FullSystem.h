...
template<typename T>
inline void
deleteOut(std::vector<T*>& v, const int i)
{
        delete v[i];
        v[i] = v.back();
        v.pop_back();
...
template<typename T>
inline void
deleteOutPt(std::vector<T*>& v, const T* i)
{
        delete i;
...
template<typename T>
inline void
deleteOutOrder(std::vector<T*>& v, const int i)
{
        delete v[i];
...
template<typename T>
inline void
deleteOutOrder(std::vector<T*>& v, const T* element)
{
        int i = -1;
        for (unsigned int k = 0; k < v.size(); k++) {
                if (v[k] == element) {
...
inline bool
eigenTestNan(const MatXX& m, std::string msg)
{
        bool foundNan = false;
...
class FullSystem
{
      public:
        ...
        void setOriginalCalib(const VecXf& originalCalib,
                              int originalW,
                              int originalH);
      ...
      private:
        ...
        double linAllPointSinle(PointHessian* point,
                                float outlierTHSlack,
                                bool plot);
        ...
        void activatePointsMT_Reductor(std::vector<PointHessian*>* optimized,
                                       ...
                                       Vec10* stats,
                                       int tid);
        void applyRes_Reductor(bool copyJacobians,
                               int min,
                               int max,
                               Vec10* stats,
                               int tid);

        void printOptRes(const Vec3& res,
                         double resL,
                         double resM,
                         double resPrior,
                         double LExact,
        ...
        // =================== changed by tracker-thread. protected by
        // trackMutex ============
        ...
        boost::mutex coarseTrackerSwapMutex;   // if tracker sees that there is a
                                               // new reference, tracker locks
                                               // [coarseTrackerSwapMutex] and
                                               // swaps the two.
        CoarseTracker* coarseTracker_forNewKF; // set as as reference. protected
        ...
        // tracking / mapping synchronization. All protected by
        // [trackMapSyncMutex].
