...
bool CoarseInitializer::trackFrame(
    ...
    std::vector<IOWrap::Output3DWrapper*>& wraps) {
        ...
        for (int lvl = pyrLevelsUsed - 1; lvl >= 0; lvl--) {
                ...
                resetPoints(lvl);
                Vec3f resOld =
                    calcResAndGS(lvl, H, b, Hsc, bsc, refToNew_current,
                ...
                while (true) {
                        ...
                        Vec8f b_new, bsc_new;
                        Vec3f resNew =
                            calcResAndGS(lvl, H_new, b_new, Hsc_new, bsc_new,
...
// depth.
Vec3f CoarseInitializer::calcResAndGS(int lvl,
                                      Mat88f& H_out,
                                      Vec8f& b_out,
                                      Mat88f& H_out_sc,
                                      Vec8f& b_out_sc,
        ...
        for (int i = 0; i < npts; i++) {
                ...
                for (int i = ((patternNum >> 2) << 2); i < patternNum; i++)
                        acc9.updateSingle(
                            (float)dp0[i], (float)dp1[i], (float)dp2[i],
...
Vec3f CoarseInitializer::calcEC(int lvl) {
        ...
        return Vec3f(couplingWeight * E.A1m[0], couplingWeight * E.A1m[1],
                     E.num);
}
...
void CoarseInitializer::makeGradients(Eigen::Vector3f** data) {
        for (int lvl = 1; lvl < pyrLevelsUsed; lvl++) {
                ...
                for (int idx = wl; idx < wl * (hl - 1); idx++) {
                        ...
                        dINew_l[idx][2] = 0.5f * (dINew_l[idx + wl][0] -
                                                  dINew_l[idx - wl][0]);
                }
        }
}
