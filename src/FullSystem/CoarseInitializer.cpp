...
bool
CoarseInitializer::trackFrame(FrameHessian* newFrameHessian,
			      std::vector<IOWrap::Output3DWrapper*>& wraps)
{
	...
	if (firstFrame->ab_exposure > 0 && newFrame->ab_exposure > 0)
		refToNew_aff_current = AffLight(
		  logf(newFrame->ab_exposure / firstFrame->ab_exposure),
	...
	for (int lvl = pyrLevelsUsed - 1; lvl >= 0; lvl--) {
		...
		while (true) {
			...
			if (fixAffine) {
				inc.head<6>() =
				  -(wM.toDenseMatrix().topLeftCorner<6, 6>() *
				    (Hl.topLeftCorner<6, 6>().ldlt().solve(
				      bl.head<6>())));
				inc.tail<2>().setZero();
			} else
				inc =
				  -(wM * (Hl.ldlt().solve(bl))); //=-H^-1 * b.

			SE3 refToNew_new =
			  SE3::exp(inc.head<6>().cast<double>()) *
			  refToNew_current;
			...
			float eTotalNew =
			  (resNew[0] + resNew[1] + regEnergy[1]);
			float eTotalOld =
			  (resOld[0] + resOld[1] + regEnergy[0]);

			bool accept = eTotalOld > eTotalNew;

			if (printDebug) {
				printf(
				  "lvl %d, it %d (l=%f) %s: %.5f + %.5f + %.5f "
				  "-> %.5f + %.5f + %.5f (%.2f->%.2f) (|inc| = "
				  "%f)! \t",
				  lvl,
				  iteration,
				  ...
				  inc.norm());
				std::cout
				  << refToNew_new.log().transpose() << " AFF "
			...
			if (!(inc.norm() > eps) ||
			    iteration >= maxIterations[lvl] || fails >= 2) {
				Mat88f H, Hsc;
				Vec8f b, bsc;
...
void
CoarseInitializer::debugPlot(int lvl,
			     std::vector<IOWrap::Output3DWrapper*>& wraps)
{
	bool needCall = false;
	for (IOWrap::Output3DWrapper* ow : wraps)
	...
	for (int i = 0; i < npts; i++) {
		...
		if (!point->isGood)
			iRImg.setPixel9(
			  point->u + 0.5f, point->v + 0.5f, Vec3b(0, 0, 0));

		else
			iRImg.setPixel9(point->u + 0.5f,
					point->v + 0.5f,
...
// depth.
Vec3f
CoarseInitializer::calcResAndGS(int lvl,
				Mat88f& H_out,
				Vec8f& b_out,
				Mat88f& H_out_sc,
				Vec8f& b_out_sc,
				...
				bool plot)
{
	...
	for (int i = 0; i < npts; i++) {
		...
		for (int idx = 0; idx < patternNum; idx++) {
			...
			Vec3f pt =
			  RKi * Vec3f(point->u + dx, point->v + dy, 1) +
			  t * point->idepth_new;
			float u = pt[0] / pt[2];
			float v = pt[1] / pt[2];
			float Ku = fxl * u + cxl;
			...
			Vec3f hitColor =
			  getInterpolatedElement33(colorNew, Ku, Kv, wl);
			// Vec3f hitColor =
			// getInterpolatedElement33BiCub(colorNew, Ku, Kv, wl);

			// float rlR = colorRef[point->u+dx + (point->v+dy) *
			// wl][0];
			float rlR = getInterpolatedElement31(
			  colorRef, point->u + dx, point->v + dy, wl);
			...
			float residual =
			  hitColor[0] - r2new_aff[0] * rlR - r2new_aff[1];
			...
			float maxstep =
			  1.0f / Vec2f(dxdd * fxl, dydd * fyl).norm();
	...
	for (int i = 0; i < npts; i++) {
		...
		acc9SC.updateSingleWeighted((float)JbBuffer_new[i][0],
					    (float)JbBuffer_new[i][1],
					    (float)JbBuffer_new[i][2],
					    (float)JbBuffer_new[i][3],
					    (float)JbBuffer_new[i][4],
					    (float)JbBuffer_new[i][5],
					    (float)JbBuffer_new[i][6],
					    (float)JbBuffer_new[i][7],
					    (float)JbBuffer_new[i][8],
...
Vec3f
CoarseInitializer::calcEC(int lvl)
{
	if (!snapped)
	...
	return Vec3f(
	  couplingWeight * E.A1m[0], couplingWeight * E.A1m[1], E.num);
}
void
CoarseInitializer::optReg(int lvl)
{
	int npts = numPoints[lvl];
	Pnt* ptsl = points[lvl];
	if (!snapped) {
	...
	for (int i = 0; i < npts; i++) {
		...
		if (nnn > 2) {
			...
			point->iR = (1 - regWeight) * point->idepth +
				    regWeight * idnn[nnn / 2];
		}
	}
}

void
CoarseInitializer::propagateUp(int srcLvl)
{
...
void
CoarseInitializer::propagateDown(int srcLvl)
{
	assert(srcLvl > 0);
	// set idepth of target
	...
	for (int i = 0; i < nptst; i++) {
		...
		if (!parent->isGood || parent->lastHessian < 0.1)
			continue;
		if (!point->isGood) {
			point->iR = point->idepth = point->idepth_new =
			  parent->iR;
			point->isGood = true;
			point->lastHessian = 0;
		} else {
			float newiR =
			  (point->iR * point->lastHessian * 2 +
...
void
CoarseInitializer::makeGradients(Eigen::Vector3f** data)
{
...
void
CoarseInitializer::setFirst(CalibHessian* HCalib, FrameHessian* newFrameHessian)
{
	...
	for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
		...
		if (lvl == 0)
			...
			npts = makePixelStatus(firstFrame->dIp[lvl],
					       statusMapB,
					       w[lvl],
					       h[lvl],
					       densities[lvl] * w[0] * h[0]);
		...
		Pnt* pl = points[lvl];
		int nl = 0;
		for (int y = patternPadding + 1; y < hl - patternPadding - 2;
		     y++)
			for (int x = patternPadding + 1;
			     x < wl - patternPadding - 2;
			     x++) {
				// if(x==2) printf("y=%d!\n",y);
...
void
CoarseInitializer::resetPoints(int lvl)
{
	Pnt* pts = points[lvl];
	int npts = numPoints[lvl];
	for (int i = 0; i < npts; i++) {
		...
		if (lvl == pyrLevelsUsed - 1 && !pts[i].isGood) {
			...
			if (sn > 0) {
				pts[i].isGood = true;
				pts[i].iR = pts[i].idepth = pts[i].idepth_new =
				  snd / sn;
			}
		}
	}
...
void
CoarseInitializer::doStep(int lvl, float lambda, Vec8f inc)
{
...
void
CoarseInitializer::applyStep(int lvl)
{
	Pnt* pts = points[lvl];
	int npts = numPoints[lvl];
	for (int i = 0; i < npts; i++) {
...
void
CoarseInitializer::makeK(CalibHessian* HCalib)
{
	w[0] = wG[0];
	h[0] = hG[0];
	...
	for (int level = 0; level < pyrLevelsUsed; ++level) {
		K[level] << fx[level], 0.0, cx[level], 0.0, fy[level],
		  cy[level], 0.0, 0.0, 1.0;
...
CoarseInitializer::makeNN()
{
	...
	for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
		...
		for (int i = 0; i < npts; i++) {
			// resultSet.init(pts[i].neighbours,
			// pts[i].neighboursDist );
			resultSet.init(ret_index, ret_dist);
			Vec2f pt = Vec2f(pts[i].u, pts[i].v);
			indexes[lvl]->findNeighbors(
			  resultSet, (float*)&pt, nanoflann::SearchParams());
			int myidx = 0;
			float sumDF = 0;
			for (int k = 0; k < nn; k++) {
