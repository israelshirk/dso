...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh) {
	...
	if (allFrameHistory.size() == 2)
		...
		for (float rotDelta = 0.02; rotDelta < 0.05; rotDelta++) {
			...
			lastF_2_fh_tries.push_back(
			    fh_2_slast.inverse() * lastF_2_slast *
			    SE3(Sophus::Quaterniond(1, -rotDelta, -rotDelta,
						    -rotDelta),
				Vec3(0, 0, 0)));  // assume constant motion.
			lastF_2_fh_tries.push_back(
			    fh_2_slast.inverse() * lastF_2_slast *
	...
	for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++) {
		...
		bool trackingIsGood = coarseTracker->trackNewestCoarse(
		    ...
		    achievedRes);  // in each level has to be at least as good
				   // as the last try.
		tryIterations++;

		if (i != 0) {
			printf(
			    "RE-TRACK ATTEMPT %d with initOption %d and "
			    "start-lvl %d (ab %f %f): %f %f %f %f %f -> %f %f "
			    "%f %f %f \n",
			    i, i, pyrLevelsUsed - 1, aff_g2l_this.a,
			    aff_g2l_this.b, achievedRes[0], achievedRes[1],
			    achievedRes[2], achievedRes[3], achievedRes[4],
		...
		if (haveOneGood &&
		    achievedRes[0] <
			lastCoarseRMSE[0] * setting_reTrackThreshold)
			break;
	}

	if (!haveOneGood) {
		printf(
		    "BIG ERROR! tracking failed entirely. Take predictred pose "
	...
	if (setting_logStuff) {
		(*coarseTrackingLog)
		    ...
		    << aff_g2l.a << " " << aff_g2l.b << " " << achievedRes[0]
		    << " " << tryIterations << "\n";
	}
...
void FullSystem::traceNewCoarse(FrameHessian* fh) {
	...
	for (FrameHessian* host :
	     frameHessians)  // go through all active frames
	{
...
void FullSystem::activatePointsMT() {
	...
	if (!setting_debugout_runquiet)
		printf(
		    "SPARSITY:  MinActDist %f (need %d points, have %d "
		    "points)!\n",
	...
	if (multiThreading)
		...
		activatePointsMT_Reductor(&optimized, &toOptimize, 0,
					  toOptimize.size(), 0, 0);
	...
	} else  // do front-end operation.
	{
		...
		if (setting_keyframesPerSecond > 0) {
			...
			Vec2 refToFh = AffLight::fromToVecExposure(
			    ...
			    fh->ab_exposure, coarseTracker->lastRef_aff_g2l,
			    fh->shell->aff_g2l);
...
void FullSystem::initializeFromInitializer(FrameHessian* newFrame) {
	...
	for (int i = 0; i < coarseInitializer->numPoints[0]; i++) {
		...
		ImmaturePoint* pt =
		    new ImmaturePoint(point->u + 0.5f, point->v + 0.5f,
...
void FullSystem::printLogLine() {
	...
	if (!setting_debugout_runquiet)
		printf(
		    "LOG %d: %.3f fine. Res: %d A, %d L, %d M; (%'d / %'d) "
		    "forceDrop. a=%f, b=%f. Window %d (%d)\n",
		    allKeyFramesHistory.back()->id,
		    statistics_lastFineTrackRMSE, ef->resInA, ef->resInL,
		    ef->resInM, (int)statistics_numForceDroppedResFwd,
