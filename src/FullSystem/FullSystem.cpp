...
FullSystem::trackNewCoarse(FrameHessian* fh)
{
    ...
    for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++) {
        ...
        bool trackingIsGood =
          coarseTracker->trackNewestCoarse(fh,
                                           lastF_2_fh_this,
                                           aff_g2l_this,
                                           pyrLevelsUsed - 1,
                                           achievedRes); // in each level has to
                                                         // be at least as good
                                                         // as the last try.
        tryIterations++;
    ...
    for (FrameHessian* host : frameHessians) // go through all active frames
    {
        ...
        Vec2f aff =
          AffLight::fromToVecExposure(
            host->ab_exposure, fh->ab_exposure, host->aff_g2l(), fh->aff_g2l())
            .cast<float>();
...
FullSystem::addActiveFrame(ImageAndExposure* image, int id)
{
    ...
    } else // do front-end operation.
    {
        ...
        if (setting_keyframesPerSecond > 0) {
            ...
            Vec2 refToFh =
              AffLight::fromToVecExposure(coarseTracker->lastRef->ab_exposure,
                                          fh->ab_exposure,
