...
FullSystem::FullSystem()
{
	...
	if (setting_logStuff) {
		...
		calibLog->open("logs/calibLog.txt",
			       std::ios::trunc | std::ios::out);
		calibLog->precision(12);

		numsLog = new std::ofstream();
		numsLog->open("logs/numsLog.txt",
			      std::ios::trunc | std::ios::out);
		...
		eigenPLog->open("logs/eigenPLog.txt",
				std::ios::trunc | std::ios::out);
		eigenPLog->precision(10);

		eigenALog = new std::ofstream();
		eigenALog->open("logs/eigenALog.txt",
				std::ios::trunc | std::ios::out);
		eigenALog->precision(10);

		DiagonalLog = new std::ofstream();
		DiagonalLog->open("logs/diagonal.txt",
				  std::ios::trunc | std::ios::out);
...
void
FullSystem::setOriginalCalib(const VecXf& originalCalib,
			     int originalW,
			     int originalH)
{
}
...
void
FullSystem::setGammaFunction(float* BInv)
{
	if (BInv == 0)
		return;
...
void
FullSystem::printResult(std::string file)
{
...
FullSystem::trackNewCoarse(FrameHessian* fh)
{
	...
	if (allFrameHistory.size() == 2)
		...
		FrameShell* sprelast =
		  allFrameHistory[allFrameHistory.size() - 3];
		...
		lastF_2_fh_tries.push_back(
		  SE3::exp(fh_2_slast.log() * 0.5).inverse() *
		  lastF_2_slast); // assume half motion.
		lastF_2_fh_tries.push_back(
		  lastF_2_slast); // assume zero motion.
		lastF_2_fh_tries.push_back(
		  SE3()); // assume zero motion FROM KF.

		// just try a TON of different initializations (all rotations).
		// In the end, if they don't work they will only be tried on the
		// coarsest level, which is super fast anyway. also, if tracking
		// rails here we loose, so we really, really want to avoid that.
		for (float rotDelta = 0.02; rotDelta < 0.05; rotDelta++) {
			...
			lastF_2_fh_tries.push_back(
			  fh_2_slast.inverse() * lastF_2_slast *
			  SE3(Sophus::Quaterniond(
				1, -rotDelta, -rotDelta, -rotDelta),
			      Vec3(0, 0, 0))); // assume constant motion.
			lastF_2_fh_tries.push_back(
			  fh_2_slast.inverse() * lastF_2_slast *
			  SE3(Sophus::Quaterniond(
	...
	// as long as maxResForImmediateAccept is not reached, I'll continue
	// through the options. I'll keep track of the so-far best achieved
	// residual for each level in achievedRes. If on a coarse level,
	// tracking is WORSE than achievedRes, we will not continue to save
	// time.
	...
	for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++) {
		...
		if (i != 0) {
			printf("RE-TRACK ATTEMPT %d with initOption %d and "
			       "start-lvl %d (ab %f %f): %f %f %f %f %f -> %f "
			       "%f %f %f %f \n",
			       i,
			       i,
			       pyrLevelsUsed - 1,
		...
		if (haveOneGood) {
			for (int i = 0; i < 5; i++) {
				if (!std::isfinite((float)achievedRes[i]) ||
				    achievedRes[i] >
				      coarseTracker
					->lastResiduals[i]) // take over if
							    // achievedRes is
							    // either bigger or
							    // NAN.
					achievedRes[i] =
					  coarseTracker->lastResiduals[i];
			}
		}
	...
	if (!haveOneGood) {
		printf("BIG ERROR! tracking failed entirely. Take predictred "
		       "pose and hope we may somehow recover.\n");
		flowVecs = Vec3(0, 0, 0);
	...
	if (setting_logStuff) {
		(*coarseTrackingLog)
		  << std::setprecision(16) << fh->shell->id << " "
		  << fh->shell->timestamp << " " << fh->ab_exposure << " "
		  << fh->shell->camToWorld.log().transpose() << " " << aff_g2l.a
		  << " " << aff_g2l.b << " " << achievedRes[0] << " "
		  << tryIterations << "\n";
	}
...
void
FullSystem::traceNewCoarse(FrameHessian* fh)
{
	...
	//	printf("ADD: TRACE: %'d points. %'d (%.0f%%) good. %'d (%.0f%%)
	// skip. %'d (%.0f%%) badcond. %'d (%.0f%%) oob. %'d (%.0f%%) out. %'d
	//(%.0f%%) uninit.\n", 			trace_total,
...
void
FullSystem::activatePointsMT_Reductor(std::vector<PointHessian*>* optimized,
				      ...
				      int tid)
{
	...
	for (int k = min; k < max; k++) {
		(*optimized)[k] =
		  optimizeImmaturePoint((*toOptimize)[k], 1, tr);
	}
	delete[] tr;
}
...
FullSystem::activatePointsMT()
{
	...
	if (!setting_debugout_runquiet)
		printf("SPARSITY:  MinActDist %f (need %d points, have %d "
		       "points)!\n",
		       currentMinActDist,
	...
	for (FrameHessian* host : frameHessians) // go through all active frames
	{
		...
		Mat33f KRKi = (coarseDistanceMap->K[1] *
			       fhToNew.rotationMatrix().cast<float>() *
			       coarseDistanceMap->Ki[0]);
		Vec3f Kt = (coarseDistanceMap->K[1] *
			    fhToNew.translation().cast<float>());

		for (unsigned int i = 0; i < host->immaturePoints.size();
		     i += 1) {
			ImmaturePoint* ph = host->immaturePoints[i];
			ph->idxInImmaturePoints = i;

			// delete points that have never been traced
			// successfully, or that are outlier on the last trace.
			...
			// if I cannot activate the point, skip it. Maybe also
			// delete it.
			if (!canActivate) {
				// if point will be out afterwards, delete it
				// instead.
			...
			if ((u > 0 && v > 0 && u < wG[1] && v < hG[1])) {
				...
				if (dist >= currentMinActDist * ph->my_type) {
					coarseDistanceMap->addIntoDistFinal(u,
									    v);
	...
	//	printf("ACTIVATE: %d. (del %d, notReady %d, marg %d, good %d,
	// marg-skip %d)\n", 			(int)toOptimize.size(),
	// immature_deleted, immature_notReady, immature_needMarg,
	// immature_want,  immature_margskip);
	...
	if (multiThreading)
		treadReduce.reduce(
		  boost::bind(&FullSystem::activatePointsMT_Reductor,
			      this,
			      &optimized,
			      &toOptimize,
			      _1,
		...
		activatePointsMT_Reductor(
		  &optimized, &toOptimize, 0, toOptimize.size(), 0, 0);
	...
	for (unsigned k = 0; k < toOptimize.size(); k++) {
		...
		if (newpoint != 0 && newpoint != (PointHessian*)((long)(-1))) {
			newpoint->host
			  ->immaturePoints[ph->idxInImmaturePoints] = 0;
...
void
FullSystem::activatePointsOldFirst()
{
	assert(false);
}
	...
	for (FrameHessian* host : frameHessians) // go through all active frames
	{
		for (unsigned int i = 0; i < host->pointHessians.size(); i++) {
			...
			if (ph->idepth_scaled < 0 ||
				...
				flag_nores++;
			} else if (ph->isOOB(fhsToKeepPoints,
					     fhsToMargPoints) ||
				...
				if (ph->isInlierNew()) {
					flag_in++;
					int ngoodRes = 0;
					for (PointFrameResidual* r :
					     ph->residuals) {
						r->resetOOB();
						r->linearize(&Hcalib);
						r->efResidual->isLinearized =
						  false;
						r->applyRes(true);
					...
					if (ph->idepth_hessian >
					    setting_minIdepthH_marg) {
						flag_inin++;
						ph->efPoint->stateFlag =
						  EFPointStatus::PS_MARGINALIZE;
						host->pointHessiansMarginalized
						  .push_back(ph);
					} else {
						ph->efPoint->stateFlag =
						  EFPointStatus::PS_DROP;
						host->pointHessiansOut
						  .push_back(ph);
					}
					...
					// printf("drop point in frame %d (%d
					// goodRes, %d activeRes)\n",
		...
		for (int i = 0; i < (int)host->pointHessians.size(); i++) {
			if (host->pointHessians[i] == 0) {
				host->pointHessians[i] =
				  host->pointHessians.back();
...
void
FullSystem::addActiveFrame(ImageAndExposure* image, int id)
{
	...
	if (!initialized) {
		...
		} else if (coarseInitializer->trackFrame(
			     fh, outputWrapper)) // if SNAPPED
		{
	...
	} else // do front-end operation.
	{
		...
		// =========================
		if (coarseTracker_forNewKF->refFrameID >
		...
		if (setting_keyframesPerSecond > 0) {
			needToMakeKF = allFrameHistory.size() == 1 ||
				       (fh->shell->timestamp -
					allKeyFramesHistory.back()->timestamp) >
...
void
FullSystem::deliverTrackedFrame(FrameHessian* fh, bool needKF)
{
...
FullSystem::mappingLoop()
{
	...
	while (runMapping) {
		...
		// guaranteed to make a KF for the very first two tracked
		// frames.
		    ...
		    0) // if there are other frames to tracke, do that first.
		{
			...
			if (needToKetchupMapping &&
			    unmappedTrackedFrames.size() > 0) {
				FrameHessian* fh =
				  unmappedTrackedFrames.front();
				unmappedTrackedFrames.pop_front();
				{
					boost::unique_lock<boost::mutex> crlock(
					  shellPoseMutex);
					...
					fh->shell->camToWorld =
					  fh->shell->trackingRef->camToWorld *
					  fh->shell->camToTrackingRef;
					fh->setEvalPT_scaled(
					  fh->shell->camToWorld.inverse(),
					  fh->shell->aff_g2l);
				}
				delete fh;
			}
...
void
FullSystem::makeNonKeyFrame(FrameHessian* fh)
{
...
void
FullSystem::makeKeyFrame(FrameHessian* fh)
{
	...
	for (FrameHessian* fh1 : frameHessians) // go through all active frames
	{
		...
		for (PointHessian* ph : fh1->pointHessians) {
			PointFrameResidual* r =
			  new PointFrameResidual(ph, fh1, fh);
...
void
FullSystem::initializeFromInitializer(FrameHessian* newFrame)
{
	boost::unique_lock<boost::mutex> lock(mapMutex);
	...
	// int numPointsTotal = makePixelStatus(firstFrame->dI, selectionMap,
	// wG[0], hG[0], setting_desiredDensity);  int numPointsTotal =
	...
	for (int i = 0; i < coarseInitializer->numPoints[0]; i++) {
		...
		ImmaturePoint* pt = new ImmaturePoint(point->u + 0.5f,
						      point->v + 0.5f,
						      firstFrame,
						      point->my_type,
						      &Hcalib);
...
void
FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth)
{
	pixelSelector->allowFast = true;
	// int numPointsTotal = makePixelStatus(newFrame->dI, selectionMap,
	// wG[0], hG[0], setting_desiredDensity);
	...
	for (int y = patternPadding + 1; y < hG[0] - patternPadding - 2; y++)
		for (int x = patternPadding + 1; x < wG[0] - patternPadding - 2;
			...
			ImmaturePoint* impt = new ImmaturePoint(
			  x, y, newFrame, selectionMap[i], &Hcalib);
...
FullSystem::printEigenValLine()
{
	...
	if (eigenALog != 0) {
		...
		(*eigenALog) << allKeyFramesHistory.back()->id << " "
			     << ea.transpose() << "\n";
		eigenALog->flush();
	}
	if (eigenPLog != 0) {
		...
		(*eigenPLog) << allKeyFramesHistory.back()->id << " "
			     << ea.transpose() << "\n";
		eigenPLog->flush();
	}
