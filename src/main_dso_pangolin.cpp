        ...
        if (1 == sscanf(arg, "save=%d", &option)) {
                if (option == 1) {
                        ...
                        if (42 == system("rm -rf images_out"))
                                printf("system call returned 42 - what are the "
                                       "odds?. This is only here to shut up "
                                       "the compiler.\n");
...
main(int argc, char** argv)
{
        ...
        // to make MacOS happy: run this in dedicated thread -- and use this one
        // to run the GUI.
        std::thread runthread([&]() {
                ...
                for (int i = lstart; i >= 0 && i < reader->getNumImages() &&
                     ...
                     i += linc) {
                        ...
                        if (timesToPlayAt.size() == 0) {
                                ...
                                double tsThis = reader->getTimestamp(
                                  idsToPlay[idsToPlay.size() - 1]);
                                double tsPrev = reader->getTimestamp(
                                  idsToPlay[idsToPlay.size() - 2]);
                ...
                for (int ii = 0; ii < (int)idsToPlay.size(); ii++) {
                        if (!fullSystem->initialized) // if not initialized:
                                                      // reset start time.
                        {
                                gettimeofday(&tv_start, NULL);
                                started = clock();
                        ...
                        if (playbackSpeed != 0) {
                                ...
                                if (sSinceStart < timesToPlayAt[ii])
                                        usleep((int)((timesToPlayAt[ii] -
                                                      sSinceStart) *
                                                     1000 * 1000));
                                else if (sSinceStart > timesToPlayAt[ii] + 0.5 +
                                                         0.1 * (ii % 2)) {
                                        printf("SKIPFRAME %d (play at %f, now "
                                               "it is %f)!\n",
                                               ii,
                                               timesToPlayAt[ii],
                                               sSinceStart);
                        ...
                        if (fullSystem->initFailed ||
                            setting_fullResetRequested) {
                                if (ii < 250 || setting_fullResetRequested) {
                                        ...
                                        std::vector<IOWrap::Output3DWrapper*>
                                          wraps = fullSystem->outputWrapper;
                                        delete fullSystem;

                                        for (IOWrap::Output3DWrapper* ow :
                                             wraps)
                                                ow->reset();

                                        fullSystem = new FullSystem();
                                        fullSystem->setGammaFunction(
                                          reader->getPhotometricGamma());
                                        fullSystem->linearizeOperation =
                                          (playbackSpeed == 0);
                ...
                if (setting_logStuff) {
                        std::ofstream tmlog;
                        tmlog.open("logs/time.txt",
                                   std::ios::trunc | std::ios::out);
                        tmlog
                          << 1000.0f * (ended - started) /
