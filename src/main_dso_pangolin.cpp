	...
	if (1 == sscanf(arg, "save=%d", &option)) {
		if (option == 1) {
			...
			if (42 == system("rm -rf images_out"))
				printf(
				    "system call returned 42 - what are the "
				    "odds?. This is only here to shut up the "
				    "compiler.\n");
...
int main(int argc, char** argv) {
	...
	if (setting_photometricCalibration > 0 &&
		...
		printf(
		    "ERROR: dont't have photometric calibation. Need to use "
